...
const RULES_GIRO_DOBLE_RECHAZO = {
  ...
  minRejectRatio: 0.090,
  minFirstRejectRatio: 0.090,
  minSecondRejectRatio: 0.050,
  minOppositeAdvanceRatio: 0.060,
  maxCloseBeyondTol: 0.55,
  maxDominantResponseRatio: 0.68,
  maxDominantResponseSlopeVsReject: 0.78,
  veryWeakDominantResponseRatio: 0.50,
  ...
  weakAngleRatio: 0.78,
  minRejectionSlopeVsApproach: 0.92,
...
function analyzeGiroSinNivelSide(pts, isResistance, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  const evalMs = Math.max(1, Number(arr[arr.length - 1]?.ms || 0));
  const allowPartialAt35 = !!rules.allowPartialAt35 && evalMs <= (Number(rules.partialEvalSec || 35) * 1000 + 800);
  const minFirstAttackMs = allowPartialAt35 ? 6500 : 8500;
  const maxFirstAttackMs = evalMs * (allowPartialAt35 ? 0.76 : 0.70);
  const openZ = arr[0].z;

  // Ataque amplio: lo principal. La primera pata tiene que tener desplazamiento real.
  const minAttackFromSwing = range * (allowPartialAt35 ? 0.28 : 0.32);
  const minAttackFromOpen = range * (allowPartialAt35 ? 0.16 : 0.20);
  const minFirstReject = range * (allowPartialAt35 ? 0.095 : 0.115);
  const minRetest = range * (allowPartialAt35 ? 0.040 : 0.050);
  const maxResponseRatio = allowPartialAt35 ? 0.82 : 0.72;
  const maxBreak = range * (allowPartialAt35 ? 0.060 : 0.050);
  let best = null;
  ...
  for (let firstTopIdx = 2; firstTopIdx <= arr.length - 5; firstTopIdx++) {
    ...
    if (attackEfficiency < 0.27) continue; // evita lateral/trabado antes del supuesto rechazo
    ...
    for (let rejectLowIdx = firstTopIdx + 1; rejectLowIdx <= arr.length - 4; rejectLowIdx++) {
      ...
      const weakBySpeed = responseSlope <= Math.max(attackSlope * 0.74, rejectionSlope * 0.82);
      const lowerHigh = secondTopZ <= firstTopZ - range * (allowPartialAt35 ? 0.015 : 0.022);
      const noStrongBreak = secondTopZ <= firstTopZ + maxBreak;
      const weakResponse = noStrongBreak && ((weakBySize && weakBySpeed) || (lowerHigh && weakBySpeed) || responseRatio <= 0.62);
      if (!weakResponse) continue;
      ...
      if (!allowPartialAt35) {
        ...
        if (afterLowIdx > secondTopIdx) {
          rejection2 = Math.max(0, secondTopZ - arr[afterLowIdx].z);
          rejection2Slope = rejection2 / Math.max(1, arr[afterLowIdx].ms - arr[secondTopIdx].ms);
          secondRejectByAngle = rejection2Slope >= responseSlope * 0.85 && rejection2 >= range * 0.030;
        }
      }
      ...
      const res = {
        ...
        stage: allowPartialAt35 ? "35s_zona_intravela_ataque_rechazo_retest_debil" : "zona_intravela_ataque_rechazo_retest_debil_completo",
        points,
        score,
      };
  ...
  return {
    ...
    meta: {
      ...
      status: seq.partial35
        ? "35s: zona intravela + ataque amplio + rechazo claro + retesteo débil"
        : "zona intravela + ataque amplio + rechazo claro + retesteo débil + continuación contraria",
      logic: seq.isResistance
        ? "zona intravela: compradores atacan fuerte; vendedores rechazan; compradores vuelven débil/lento => PUT"
        : "zona intravela: vendedores atacan fuerte; compradores rechazan; vendedores vuelven débil/lento => CALL",
    },
  };
}
