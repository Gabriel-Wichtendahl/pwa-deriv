...
async function findBestHighLowPlan(item, side, opts = {}) {
  ...
  // Nueva prioridad: usar la barrera por defecto que Deriv trae para ese par.
  // Esto NO busca 120% ni prueba una lista fija como barrera principal.
  try {
    const defaultPlan = await getHighLowDefaultProposalQuote(symbol, side, stake, timeoutMs);
    if (defaultPlan?.proposalId) {
      rememberExecutionBarrierHint(symbol, side, defaultPlan, defaultPlan.precision || 0);
      return defaultPlan;
    }
  } catch (e) {
    // Algunos contratos/símbolos pueden exigir barrier explícito. En ese caso se usa fallback.
  }

  // Fallback: se conserva para no dejar sin operar si Deriv exige barrier explícito.
  // Ya no se usa para buscar 120%; solo toma la primera propuesta válida.
...
async function refreshExecutionPlanForSignal(item, force = false) {
  ...
  cache.running = (async () => {
    ...
    try {
      ...
      const callPlan = await findBestHighLowPlan(item, "CALL", { fast: true });
      let putPlan = callPlan ? await findMirroredHighLowPlan(item, "PUT", callPlan, { fast: true }) : null;
      if (!putPlan) putPlan = await findBestHighLowPlan(item, "PUT", { fast: true });
      cache.call = callPlan;
      cache.put = putPlan;
...
function applyModalExecutionButtonUI(locked = false, candleClosed = false) {
  ...
  const applyState = (btn, plan, sideLabel) => {
    ...
    if (plan) {
      btn.title = `${sideLabel} listo | barrera ${plan.barrier || "auto del par"} | payout ${Math.round(plan.profitPct)}%`;
      return;
    }
  ...
  try {
    ...
    if (shouldUseAutoHighLowExecution() && itemCtx?.id) {
      ...
      if (!plan?.proposalId || !Number.isFinite(plan.askPrice)) {
        throw new Error(`No encontré propuesta válida para ${side === "CALL" ? "HIGHER" : "LOWER"} con barrera del par`);
      }
