...
const AUTO_PRECALC_FAST_PIPS = [60, 80, 100, 120, 150, 180, 200, 220, 250, 285, 320, 350, 400, 450, 500, 650, 800, 1000, 1200];
...
function getExecutionModeLabel() {
  return shouldUseAutoHighLowExecution() ? "🎯 High/Low barrera relativa del par" : "↕️ Rise/Fall 1m";
}
function applyExecutionModeUI() {
  ...
  btn.title = shouldUseAutoHighLowExecution()
    ? "Usa barrera relativa del par: COMPRA = HIGHER y VENTA = LOWER con la misma distancia en negativo."
...
async function getHighLowDefaultProposalQuote(symbol, side, stake, timeoutMs = AUTO_FULL_PROPOSAL_TIMEOUT_MS) {
  ...
  if (res?.error) throw new Error(res.error.message || "proposal default error");
...
async function findBestHighLowPlan(item, side, opts = {}) {
  ...
  // Higher/Lower en la API de Deriv se compra como CALL/PUT + barrier.
  // Sin barrier, CALL/PUT puede convertirse en Rise/Fall; por eso NO usamos proposal sin barrera.
  // Importante: no tiramos 50+ proposals juntas porque Deriv/WS puede no devolver ninguna por rate/timeout.
  // Probamos por tandas chicas y devolvemos la primera propuesta aceptada por ese símbolo.
  const { coarse } = buildBarrierCandidates(item, side, fast ? "fast" : "full");
  if (!coarse.length) return null;

  const batchSize = fast ? 6 : 8;
  for (let i = 0; i < coarse.length; i += batchSize) {
    const batch = coarse.slice(i, i + batchSize);
    const results = await Promise.allSettled(
      batch.map((candidate) => getHighLowProposalQuote(symbol, side, candidate, candidate?.precision || 0, stake, timeoutMs))
    );
    const ok = results
...
async function refreshExecutionPlanForSignal(item, force = false) {
  ...
  cache.running = (async () => {
    ...
    try {
      ...
      const callPlan = await findBestHighLowPlan(item, "CALL", { fast: false });
      let putPlan = callPlan ? await findMirroredHighLowPlan(item, "PUT", callPlan, { fast: false }) : null;
      if (!putPlan) putPlan = await findBestHighLowPlan(item, "PUT", { fast: false });
      cache.call = callPlan;
      cache.put = putPlan;
...
async function ensureExecutionPlanForTrade(item, side) {
  ...
  if (side === "PUT") {
    ...
    if (!reference) {
      reference = await findBestHighLowPlan(item, "CALL", { fast: true });
      if (!reference) reference = await findBestHighLowPlan(item, "CALL", { fast: false });
      if (reference) {
    ...
    if (!quick) quick = await findMirroredHighLowPlan(item, "PUT", reference, { fast: false });
  }
  if (!quick) quick = await findBestHighLowPlan(item, side, { fast: true });
  if (!quick) quick = await findBestHighLowPlan(item, side, { fast: false });
  if (quick) {
...
function formatExecutionPlanMini(plan) {
  if (!plan) return "…";
  const mirror = plan.mirroredBarrier ? " ↔" : plan.symbolDefaultBarrier ? " auto" : "";
  const pct = Number.isFinite(Number(plan.profitPct)) ? ` · ${Math.round(plan.profitPct)}%` : "";
...
function applyModalExecutionButtonUI(locked = false, candleClosed = false) {
  ...
  const applyState = (btn, plan, sideLabel) => {
    ...
    if (plan) {
      btn.title = `${sideLabel} listo | API ${plan.apiContractType || (sideLabel === "HIGHER" ? "CALL" : "PUT")} + barrier ${plan.barrier || "auto"} | payout ${Math.round(plan.profitPct)}%`;
      return;
    }
...
function ensurePracticeReady() {
  ...
  if (!practiceRound) {
    ...
    setPracticeConfirmationControlsVisible(false);
    updatePracticeExportSaveButtonUI();
    updatePracticeResult(`Se usa solo el pool de Giro Doble Rechazo: trades de ese modo + señales guardadas con 💾. PASAR no entra en el porcentaje. Auto-entrada: ${PRACTICE_AUTO_ENTRY_SEC}s con ${PRACTICE_CONFIRM_MIN} puntos netos.`, "is-pass");
    setPracticePassButtonMode("NEXT");
    setPracticeDecisionState(true);
...
const PENDING_CONTRACT_WATCHDOG_INTERVAL_MS = 4500;
const PENDING_CONTRACT_WATCHDOG_TIMEOUT_MS = 12000;
...
function startPendingContractWatchdog({ immediate = false } = {}) {
  try {
    ...
    if (immediate) setTimeout(() => pollPendingContractsOnce("immediate"), 250);
  } catch {}
}
...
async function pollPendingContractsOnce(sourceLabel = "watchdog") {
  try {
    ...
    try {
      for (const cid of list) {
        if (!(disciplinePendingContracts || []).map(String).includes(cid)) continue;
        await pollOnePendingContract(cid, sourceLabel);
      }
    } finally {
      pendingContractWatchdogRunning = false;
  ...
  try {
    ...
    setTimeout(async () => {
      try {
        ...
        if (poc.is_sold) {
          applyClosedContractOutcomeFromPOC(poc, "fallback");
        }
      } catch {}
    }, delayMs);
  } catch {}
  ...
  try {
    ...
    if (shouldUseAutoHighLowExecution() && itemCtx?.id) {
      ...
      if (!plan?.proposalId || !Number.isFinite(plan.askPrice)) {
        // Fallback seguro: si Deriv no acepta Higher/Lower para ese símbolo/duración/barrera,
        // no perdemos la entrada. Usamos Rise/Fall normal con el mismo lado.
        const hlName = side === "CALL" ? "HIGHER" : "LOWER";
        toast(`⚠️ ${hlName} sin barrera relativa válida · entro Rise/Fall`, 1600);
        ...
        if (proposalRes?.error) throw new Error(proposalRes.error.message || "proposal Rise/Fall fallback error");
        ...
        if (!proposalId || !Number.isFinite(askPrice) || askPrice <= 0 || !Number.isFinite(payout)) {
          throw new Error(`No encontré propuesta válida para ${hlName} ni fallback Rise/Fall.`);
        }
        const profitPct = ((payout - askPrice) / askPrice) * 100;
        ...
        tradeExtra = {
          ...tradeExtra,
          exec_mode: "HIGHLOW_NO_VALID_BARRIER_FALLBACK_RISE_FALL",
          requested_exec_mode: executionMode,
          requested_contract_type: hlName,
          contract_type: side,
          barrier: "",
          highlow_fallback: true,
          fallback_reason: "sin_propuesta_highlow_barrera_relativa",
