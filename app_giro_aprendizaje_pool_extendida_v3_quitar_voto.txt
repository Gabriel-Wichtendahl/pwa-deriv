...
// ✅ FIX: en Señales y Trades se puede quitar 👍/👎 tocando de nuevo el mismo botón
...
function saveGiroAprendizajeExamples(arr = giroAprendizajeExamples) {
  try {
    ...
    try { invalidateGiroAprendizajePrototypeCache(); } catch {}
  } catch {}
}
...
function getMergedPracticeEntries() {
  ...
  // También se suman las formaciones claras guardadas desde Práctica.
  // La deduplicación final por vela se hace en getEligiblePracticeEntries().
  try {
    for (const raw of getPracticeExportSavedList()) {
      if (!raw || !Array.isArray(raw.ticks) || raw.ticks.length < 6) continue;
      const signalId = String(raw.source_signal_id || raw.id || "");
      if (signalId && seenSignalIds.has(signalId)) continue;
      out.push({
        ...raw,
        practice_id: String(raw.practice_id || raw.export_id || `CLEAR::${signalId || raw.minute || Date.now()}`),
        source_type: "practice_clear",
      });
    }
  } catch {}
...
function ensureSplitClearButtons() {
  ...
  let rebuildGiroLearningBtn = document.getElementById("rebuildGiroAprendizajePoolBtn");
  if (!rebuildGiroLearningBtn) {
    rebuildGiroLearningBtn = document.createElement("button");
    rebuildGiroLearningBtn.id = "rebuildGiroAprendizajePoolBtn";
    rebuildGiroLearningBtn.type = "button";
    rebuildGiroLearningBtn.className = "btn btnGhost";
    rebuildGiroLearningBtn.textContent = "🔄 Reconstruir pool aprendizaje";
    rebuildGiroLearningBtn.title = "Recalcula la pool extendida de Giro + Aprendizaje con likes, claras y similares.";
    host.appendChild(rebuildGiroLearningBtn);
  }
  rebuildGiroLearningBtn.onclick = () => {
    try { invalidateGiroAprendizajePrototypeCache(); } catch {}
    const stats = getGiroAprendizajePoolRuntimeStats();
    updateGiroAprendizajeControlsUI();
    toast(`🔄 Pool aprendizaje: ${stats.positives} positivas (+${stats.autoSimilar}) · ${stats.negatives} no · ${stats.candidatePool} candidatas`, 2600);
  };
...
function updateGiroAprendizajeControlsUI() {
  ...
  const poolStats = getGiroAprendizajePoolRuntimeStats();
  ...
  if (giroAprendizajeCountEl) {
    giroAprendizajeCountEl.textContent = `🧠 Giro + Aprendizaje · base ${poolStats.basePositive} · pool ${poolStats.positives} (+${poolStats.autoSimilar}) · no ${poolStats.negatives}`;
  ...
  if (giroAprendizajeHintEl) {
    const savedText = `${stats.clear + stats.target} sí / ${stats.avoid} no / ${stats.doubt} dudosas`;
    giroAprendizajeHintEl.textContent = currentLabel
      ? `Esta vela: ${getGiroAprendizajeLabelText(currentLabel)} · ${savedText}`
      : `Enseñá si esta forma es la esencia buscada · candidatos ${poolStats.candidatePool}`;
...
function buildRow(item, opts = {}) {
  ...
  // ✅ Señales y Trades: permitir tocar el mismo 👍/👎 para quitar la marca,
  // y tocar el contrario para cambiarla. Solo se bloquea si se pasa lockVote explícito.
  const voteIsLocked = !!opts.lockVote;
  const canToggleVote = !voteIsLocked;
  ...
  if (!opts.hideActions) {
    ...
    row.querySelectorAll("button[data-v]").forEach((btn) => {
      btn.onclick = (e) => {
        ...
        if (!canToggleVote) return;

        const selectedVote = String(btn.dataset.v || "");

        // ✅ En Señales y Trades: si tocás el mismo botón, se quita.
...
const RULES_GIRO_APRENDIZAJE = {
  // Más flexible: antes necesitaba 3 prototipos positivos y filtros duros,
  // por eso el modo aprendizaje podía quedarse con muy pocas velas.
  minPositivePrototypes: 2,
  sampleCount: 28,
  topSimilarityMin: 52,
  avgTop3SimilarityMin: 48,
  negativeSimilarityMax: 76,
  bodyVsRangeMin: 0.035,
  leadMoveMin: 0.20,
  closeDominanceMin: 0.42,
  releaseFromExtremeMax: 0.72,
  lateAgainstMax: 0.62,
  retraceMax: 1.05,
  dirRatioMin: 0.18,
  minPoints: 4,
  ...
  // Pool extendida: toma tus ejemplos claros y busca velas parecidas dentro
  // del journal/historial para no depender solo de las marcadas manualmente.
  autoSimilarMin: 61,
  autoSimilarNegativeGap: 5,
  maxAutoSimilarPerPositive: 8,
  maxExtendedPrototypes: 160,
};

let giroAprendizajeLastPoolStats = {
  basePositive: 0,
  positives: 0,
  negatives: 0,
  autoSimilar: 0,
...
function getGiroAprendizajeEntryKey(entry) {
  if (!entry) return "";
  return String(
    entry.source_key ||
      entry.export_id ||
      ...
      entry.practice_id ||
      entry.id ||
      getPracticeCandleKey(entry) ||
      ""
  );
}

function pushGiroAprendizajeEntry(out, seen, entry, label, source) {
  if (!entry || !Array.isArray(entry.ticks) || entry.ticks.length < 6) return false;
  const key = getGiroAprendizajeEntryKey(entry);
  const candleKey = getPracticeCandleKey(entry) || key;
  if (!key || !candleKey || seen.has(candleKey)) return false;
  seen.add(candleKey);
  out.push({
    ...entry,
    source_key: entry.source_key || key,
    aprendizajeLabel: normalizeGiroAprendizajeLabel(label),
...
function getGiroAprendizajePrototypeEntries() {
  ...
  // 1) Trades marcados: like = positivo, dislike = negativo.
  for (const entry of tradesJournal || []) {
    if (!entry) continue;
    if (entry.vote === "like") pushGiroAprendizajeEntry(out, seen, entry, "clear", "trades_like");
    if (entry.vote === "dislike") pushGiroAprendizajeEntry(out, seen, entry, "avoid", "trades_dislike");
  }

  // 2) Formaciones claras guardadas en Práctica: antes no entraban como base directa.
  for (const entry of getPracticeExportSavedList()) {
    pushGiroAprendizajeEntry(out, seen, entry, "clear", "practice_clear");
  }

  // 3) Botones del modal “Es mi formación / No es / Dudosa / Muy clara”.
  for (const ex of giroAprendizajeExamples || []) {
    ...
    if (label === "clear" || label === "target" || label === "avoid" || label === "doubt") {
      pushGiroAprendizajeEntry(out, seen, ex, label, "giro_aprendizaje");
    }
  }
...
function getGiroAprendizajeExpansionCandidateEntries() {
  const out = [];
  const seen = new Set();
  const push = (entry, source) => {
    if (!entry || entry.vote === "dislike") return;
    if (!Array.isArray(entry.ticks) || entry.ticks.length < 6) return;
    if (!(entry.nextOutcome === "up" || entry.nextOutcome === "down")) return;
    pushGiroAprendizajeEntry(out, seen, entry, "target", source);
  };

  for (const entry of getMergedPracticeEntries()) push(entry, "practice_pool");
  for (const entry of history || []) {
    if (!entry?.minuteComplete) continue;
    push(entry, "signals_history");
  }
  for (const entry of getPracticeExportSavedList()) push(entry, "practice_clear_pool");

  return out;
}

function buildGiroAprendizajePrototypeFromEntry(entry, label, evalMs, rules = RULES_GIRO_APRENDIZAJE) {
  const safeLabel = normalizeGiroAprendizajeLabel(label || entry?.aprendizajeLabel || entry?.label);
  ...
  return {
    id: getGiroAprendizajeEntryKey(entry),
...
function extendGiroAprendizajePositivePrototypes(positives, negatives, evalMs, rules = RULES_GIRO_APRENDIZAJE) {
  const base = Array.isArray(positives) ? positives : [];
  if (!base.length) return [];

  const existing = new Set(base.map((p) => String(p.id || "")).filter(Boolean));
  const candidates = getGiroAprendizajeExpansionCandidateEntries();
  const maxExtra = Math.max(0, Math.min(
    Number(rules.maxExtendedPrototypes || 0) - base.length,
    base.length * Number(rules.maxAutoSimilarPerPositive || 0)
  ));
  if (!maxExtra) return [];
  ...
  for (const entry of candidates) {
    ...
    const proto = buildGiroAprendizajePrototypeFromEntry(entry, "target", evalMs, rules);
    if (!proto) continue;

    // Filtro suave de esencia para evitar sumar ruido a la pool extendida.
    const sig = proto.sig;
    if (sig.leadMove < rules.leadMoveMin * 0.78) continue;
    if (sig.closeDominance < rules.closeDominanceMin * 0.78) continue;
    if (sig.lateAgainst > rules.lateAgainstMax + 0.16) continue;
    if (sig.dirRatio < rules.dirRatioMin * 0.65) continue;

    const sameSideBase = base.filter((p) => p.leadSign === proto.leadSign);
    const sourcePool = sameSideBase.length ? sameSideBase : base;
    const sims = sourcePool.map((p) => computeLikeMantenidoSimilarity(proto.sig, p.sig)).sort((a, b) => b - a);
    const topSimilarity = sims[0] || 0;

    const negSameSide = (negatives || []).filter((p) => p.leadSign === proto.leadSign);
    const negativeTop = negSameSide
      .map((p) => computeLikeMantenidoSimilarity(proto.sig, p.sig))
      .sort((a, b) => b - a)[0] || 0;

    if (topSimilarity < Number(rules.autoSimilarMin || 0)) continue;
    if (negativeTop && topSimilarity - negativeTop < Number(rules.autoSimilarNegativeGap || 0)) continue;

    proto.label = "target";
    proto.source = "auto_similar";
    proto.autoSimilar = true;
    proto.autoSimilarity = topSimilarity;
    proto.negativeTop = negativeTop;
    extras.push(proto);
    existing.add(key);
  }

  return extras.sort((a, b) => b.autoSimilarity - a.autoSimilarity).slice(0, maxExtra);
}

function getGiroAprendizajePoolRuntimeStats() {
  try {
    getGiroAprendizajePrototypesCached(EVAL_SEC * 1000, RULES_GIRO_APRENDIZAJE);
  } catch {}
  return giroAprendizajeLastPoolStats || { basePositive: 0, positives: 0, negatives: 0, autoSimilar: 0, candidatePool: 0 };
}
...
function getGiroAprendizajePrototypes(evalMs, rules = RULES_GIRO_APRENDIZAJE) {
  ...
  for (const entry of getGiroAprendizajePrototypeEntries()) {
    ...
    const proto = buildGiroAprendizajePrototypeFromEntry(entry, label, evalMs, rules);
    if (!proto) continue;
  ...
  const basePositive = positives.length;
  const autoExtras = extendGiroAprendizajePositivePrototypes(positives, negatives, evalMs, rules);
  if (autoExtras.length) positives.push(...autoExtras);

  giroAprendizajeLastPoolStats = {
    basePositive,
    positives: positives.length,
    negatives: negatives.length,
    autoSimilar: autoExtras.length,
    candidatePool: getGiroAprendizajeExpansionCandidateEntries().length,
  };

  return { positives, negatives };
}

let giroAprendizajePrototypeCache = { key: "", data: null };
function getGiroAprendizajePrototypeCacheKey(evalMs) {
  const markedTrades = (tradesJournal || []).filter((x) => x && (x.vote || x.comment)).length;
  return [
    Math.round(Number(evalMs || 0)),
    (tradesJournal || []).length,
    markedTrades,
    getPracticeExportSavedList().length,
    (giroAprendizajeExamples || []).length,
    (history || []).length,
    EVAL_SEC,
  ].join("|");
}
function getGiroAprendizajePrototypesCached(evalMs, rules = RULES_GIRO_APRENDIZAJE) {
  const key = getGiroAprendizajePrototypeCacheKey(evalMs);
  if (giroAprendizajePrototypeCache.key === key && giroAprendizajePrototypeCache.data) {
    return giroAprendizajePrototypeCache.data;
  }
  const data = getGiroAprendizajePrototypes(evalMs, rules);
  giroAprendizajePrototypeCache = { key, data };
  return data;
}
function invalidateGiroAprendizajePrototypeCache() {
  giroAprendizajePrototypeCache = { key: "", data: null };
}
...
function analyzeGiroAprendizajeCandidate(candidate, rules = RULES_GIRO_APRENDIZAJE) {
  const evalMs = EVAL_SEC * 1000;
  const { positives, negatives } = getGiroAprendizajePrototypesCached(evalMs, rules);
