// app.js — MODO ÚNICO: GIRO DOBLE RECHAZO/NIVEL + IA LOCAL POR EJEMPLOS + IC2 + disciplina visible + auto57 demo/real
...
function drawDerivLikeChart(canvas, ticks) {
  ...
  if (modalCurrentItem?.giroPolaridad && Number.isFinite(Number(modalCurrentItem.giroPolaridad.level))) {
    ...
    ctx.font = "12px system-ui, sans-serif";
    const roleTxt = pol.originalType ? `${pol.originalType === "support" ? "SOP." : "RES."}→${pol.levelType === "support" ? "SOP." : "RES."}` : `${pol.levelType === "support" ? "SOPORTE" : "RESIST."}`;
...
function requestModalDraw(force = false) {
  ...
  modalDrawRaf = requestAnimationFrame(() => {
    ...
    if (modalSub) {
      ...
      const manualState = normalizeManualGiroState(it.manualGiro);
      const manualTag = manualState && (manualState.favor || manualState.contra) ? ` | MANUAL:${getManualGiroStatusText(manualState.status).replace(/^[^A-ZÁÉÍÓÚÑ]*/, "")} F${manualState.favor}/C${manualState.contra}/D${manualState.diff}` : "";
...
function toggleManualGiroButton(key) {
  if (!modalCurrentItem) return;
  const state = ensureManualGiroStateForCurrentItem();
  if (!state?.selected || !(key in state.selected)) return;
  state.selected[key] = !state.selected[key];
  modalCurrentItem.manualGiro = normalizeManualGiroState(state);
  persistManualGiroForItem(modalCurrentItem);
  updateManualGiroControlsUI();
  try { requestModalDraw(true); } catch {}
}
function makeManualGiroButton(cfg) {
  const btn = document.createElement("button");
  btn.type = "button";
  btn.className = "btn btnGhost manualGiroBtn manualGiro-" + cfg.kind;
  btn.dataset.manualGiroKey = cfg.key;
  btn.textContent = cfg.label;
  btn.title = "Tocar para marcar/desmarcar este motivo manual";
  btn.style.minHeight = "34px";
  btn.style.borderRadius = "12px";
  btn.style.fontWeight = "900";
  btn.style.fontSize = "11px";
  btn.style.padding = "7px 7px";
  btn.style.lineHeight = "1.12";
  btn.style.whiteSpace = "normal";
  btn.style.touchAction = "manipulation";
  btn.style.border = "1px solid rgba(255,255,255,.13)";
  btn.style.background = "rgba(255,255,255,.035)";
  btn.onclick = (e) => {
    e.stopPropagation();
    toggleManualGiroButton(cfg.key);
  };
  return btn;
}
function ensureManualGiroControls() {
  if (manualGiroPanelEl && manualGiroPanelEl.isConnected) return manualGiroPanelEl;
  const footer =
    document.querySelector("#chartModal .modalFooter") ||
    (chartModal ? chartModal.querySelector(".modalFooter") : null);
  if (!footer) return null;
  ...
  const panel = document.createElement("div");
  panel.id = "manualGiroPanel";
  panel.style.width = "100%";
  panel.style.boxSizing = "border-box";
  panel.style.margin = "0 0 10px 0";
  panel.style.padding = "10px";
  panel.style.borderRadius = "16px";
  panel.style.border = "1px solid rgba(168,85,247,.26)";
  panel.style.background = "linear-gradient(180deg, rgba(168,85,247,.095), rgba(255,255,255,.028))";
  panel.style.boxShadow = "0 10px 22px rgba(0,0,0,.16), inset 0 0 0 1px rgba(168,85,247,.035)";
  ...
  state.id = "manualGiroState";
  state.style.fontWeight = "950";
  state.style.fontSize = "12px";
  state.style.textAlign = "right";
  state.style.lineHeight = "1.15";

  top.appendChild(title);
  top.appendChild(state);

  const summary = document.createElement("div");
  summary.id = "manualGiroSummary";
  ...
  const makeChip = (id, text) => {
    const el = document.createElement("div");
    el.dataset.manualSummary = id;
    el.textContent = text;
    el.style.padding = "6px 7px";
    el.style.borderRadius = "999px";
    el.style.border = "1px solid rgba(255,255,255,.12)";
    el.style.background = "rgba(0,0,0,.14)";
    el.style.textAlign = "center";
    return el;
  };
  summary.appendChild(makeChip("favor", "Favor 0"));
  summary.appendChild(makeChip("contra", "Contra 0"));
  summary.appendChild(makeChip("diff", "Dif 0"));

  const body = document.createElement("div");
  body.id = "manualGiroButtons";
  ...
  const group = (label, kind) => {
    ...
    h.style.opacity = ".78";
    h.style.margin = "0 0 5px 2px";
    const grid = document.createElement("div");
    grid.style.display = "grid";
    grid.style.gridTemplateColumns = "1fr";
    grid.style.gap = "5px";
    for (const cfg of MANUAL_GIRO_BUTTONS.filter((x) => x.kind === kind)) grid.appendChild(makeManualGiroButton(cfg));
    box.appendChild(h);
    box.appendChild(grid);
    return box;
  };
  body.appendChild(group("A favor del giro", "favor"));
  body.appendChild(group("En contra del giro", "contra"));

  panel.appendChild(top);
  panel.appendChild(summary);
  panel.appendChild(body);

  const manualPanel = ensureManualGiroControls();
  const signalPanel = ensureSignalConfirmationControls();
  if (manualPanel && manualPanel.parentElement === footer) manualPanel.insertAdjacentElement("afterend", panel);
  else if (signalPanel && signalPanel.parentElement === footer) signalPanel.insertAdjacentElement("afterend", panel);
  else footer.prepend(panel);

  manualGiroPanelEl = panel;
  manualGiroSummaryEl = summary;
  manualGiroStateEl = state;
  manualGiroButtonsEl = body;
  updateManualGiroControlsUI();
  return panel;
}
function updateManualGiroControlsUI() {
  if (!manualGiroPanelEl || !manualGiroPanelEl.isConnected) return;
  const state = ensureManualGiroStateForCurrentItem() || createDefaultManualGiroState();
  const statusText = getManualGiroStatusText(state.status);
  const statusColor = getManualGiroStatusColor(state.status);

  if (manualGiroStateEl) {
    manualGiroStateEl.textContent = statusText;
    manualGiroStateEl.style.color = statusColor;
    manualGiroStateEl.style.textShadow = state.status === "FORCING" ? "0 0 14px rgba(251,146,60,.45)" : "none";
  }
  if (manualGiroSummaryEl) {
    const favor = manualGiroSummaryEl.querySelector('[data-manual-summary="favor"]');
    const contra = manualGiroSummaryEl.querySelector('[data-manual-summary="contra"]');
    const diff = manualGiroSummaryEl.querySelector('[data-manual-summary="diff"]');
    if (favor) {
      favor.textContent = `Favor ${state.favor}`;
      favor.style.color = state.favor > 0 ? "#bbf7d0" : "rgba(255,255,255,.82)";
      favor.style.borderColor = state.favor > 0 ? "rgba(34,197,94,.45)" : "rgba(255,255,255,.12)";
    }
    if (contra) {
      contra.textContent = `Contra ${state.contra}`;
      contra.style.color = state.contra > 0 ? "#fed7aa" : "rgba(255,255,255,.82)";
      contra.style.borderColor = state.contra > 0 ? "rgba(251,146,60,.48)" : "rgba(255,255,255,.12)";
    }
    if (diff) {
      diff.textContent = `Dif ${state.diff}`;
      diff.style.color = state.diff >= 3 ? "#bbf7d0" : state.diff <= 1 && (state.favor + state.contra) > 0 ? "#fecaca" : "rgba(255,255,255,.82)";
      diff.style.borderColor = state.diff >= 3 ? "rgba(34,197,94,.45)" : state.diff <= 1 && (state.favor + state.contra) > 0 ? "rgba(239,68,68,.45)" : "rgba(255,255,255,.12)";
    }
  }
  if (manualGiroButtonsEl) {
    manualGiroButtonsEl.querySelectorAll(".manualGiroBtn").forEach((btn) => {
      const key = btn.dataset.manualGiroKey || "";
      const cfg = MANUAL_GIRO_BUTTONS.find((x) => x.key === key);
      const active = !!state.selected?.[key];
      btn.classList.toggle("selected", active);
      btn.setAttribute("aria-pressed", active ? "true" : "false");
      btn.style.opacity = active ? "1" : ".86";
      if (cfg?.danger && active) {
        btn.style.borderColor = "rgba(251,146,60,.95)";
        btn.style.background = "linear-gradient(180deg, rgba(251,146,60,.34), rgba(127,29,29,.18))";
        btn.style.boxShadow = "0 0 16px rgba(251,146,60,.38), inset 0 0 0 1px rgba(251,146,60,.20)";
        btn.style.color = "#fff7ed";
      } else if (cfg?.kind === "favor" && active) {
        btn.style.borderColor = "rgba(34,197,94,.82)";
        btn.style.background = "linear-gradient(180deg, rgba(34,197,94,.25), rgba(34,197,94,.09))";
        btn.style.boxShadow = "0 0 12px rgba(34,197,94,.24), inset 0 0 0 1px rgba(34,197,94,.14)";
        btn.style.color = "#dcfce7";
      } else if (cfg?.kind === "contra" && active) {
        btn.style.borderColor = "rgba(239,68,68,.78)";
        btn.style.background = "linear-gradient(180deg, rgba(239,68,68,.22), rgba(251,146,60,.08))";
        btn.style.boxShadow = "0 0 12px rgba(239,68,68,.22), inset 0 0 0 1px rgba(239,68,68,.12)";
...
function setManualGiroControlsVisible(show) {
  ensureManualGiroControls();
  if (manualGiroPanelEl) manualGiroPanelEl.style.display = show ? "block" : "none";
}
...
function updateGiroAprendizajeControlsUI() {
  ...
  if (giroAprendizajeCountEl) {
    giroAprendizajeCountEl.textContent = `🧠 IA local · ${stats.clear + stats.target} sí / ${stats.avoid} no`;
  ...
  if (giroAprendizajeHintEl) {
    giroAprendizajeHintEl.textContent = currentLabel ? `Esta vela: ${getGiroAprendizajeLabelText(currentLabel)}` : "Enseñá con ejemplos: esta forma sí/no/dudosa/muy clara";
...
function openChartModal(item) {
  ...
  setSignalConfirmationControlsVisible(true);
  ensureManualGiroControls();
  setManualGiroControlsVisible(true);
  ensureGiroAprendizajeControls();
  setGiroAprendizajeControlsVisible(true);
  updateSignalConfirmationUI();
  updateManualGiroControlsUI();
...
function closeChartModal() {
  ...
  setSignalConfirmationControlsVisible(false);
  setManualGiroControlsVisible(false);
}
...
const RULES_GIRO_DOBLE_RECHAZO = {
  // v6: más sensible. La señal puede nacer con primer rechazo/debilidad,
  // y el segundo rechazo queda como confirmación fuerte/bonus.
  minPoints: 4,
  minQualityGap: 4,
  zoneMult: 2.15,
  zoneRangeMult: 0.30,
  minApproachRatio: 0.12,
  minRejectRatio: 0.055,
  maxCloseBeyondTol: 0.72,
  minDoubleRejectPullbackRatio: 0.055,
  minSecondRejectRatio: 0.045,
  maxSecondBreakTol: 0.95,
  minClusterGap: 1,
  minClusterSize: 1,
};
function getGiroNivelSimpleCandidateLevels(symbol, minute, currentRange, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  return clusterGiroPolarityLevels(raw, tol * 1.2).map((lvl) => ({
    ...lvl,
    levelMode: "simple",
...
function analyzeGiroNivelDoubleRejectPattern(pts, level, isResistance, zone, tol, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  const arr = (pts || []).map((p, idx) => ({ idx, quote: Number(p.quote), ms: Number(p.ms || 0) })).filter((p) => Number.isFinite(p.quote));
  if (arr.length < 6) return null;

  const near = [];
  for (let i = 0; i < arr.length; i++) {
    const q = arr[i].quote;
    const touchesZone = isResistance ? q >= level - zone : q <= level + zone;
    if (touchesZone) near.push(i);
  }
  if (near.length < 2) return null;

  const clusters = [];
  let cur = [near[0]];
  const minGap = Math.max(1, Number(rules.minClusterGap || 2));
  for (let i = 1; i < near.length; i++) {
    if (near[i] - near[i - 1] <= 1) cur.push(near[i]);
    else {
      clusters.push(cur);
      cur = [near[i]];
    }
  ...
  const filtered = clusters.filter((cl) => cl.length >= Math.max(1, Number(rules.minClusterSize || 1)));
  if (filtered.length < 2) return null;

  let chosen = null;
  for (let a = 0; a < filtered.length - 1; a++) {
    for (let b = a + 1; b < filtered.length; b++) {
      const c1 = filtered[a];
      const c2 = filtered[b];
      if ((c2[0] - c1[c1.length - 1]) < minGap) continue;
      const firstExtreme = isResistance
        ? Math.max(...c1.map((i) => arr[i].quote))
        : Math.min(...c1.map((i) => arr[i].quote));
      const secondExtreme = isResistance
        ? Math.max(...c2.map((i) => arr[i].quote))
        : Math.min(...c2.map((i) => arr[i].quote));
      const between = arr.slice(c1[c1.length - 1] + 1, c2[0]);
      if (!between.length) continue;
      const pullbackMid = isResistance
        ? Math.min(...between.map((p) => p.quote))
        : Math.max(...between.map((p) => p.quote));
      const rejection1 = isResistance ? firstExtreme - pullbackMid : pullbackMid - firstExtreme;
      const rejection2 = isResistance ? secondExtreme - arr[arr.length - 1].quote : arr[arr.length - 1].quote - secondExtreme;
      const maxBreak = isResistance ? secondExtreme - level : level - secondExtreme;
      const secondNotBreak = maxBreak <= tol * Number(rules.maxSecondBreakTol || 0.60);
      const firstNear = Math.abs(firstExtreme - level) <= zone * 1.1;
      const secondNear = Math.abs(secondExtreme - level) <= zone * 1.1;
      const minPullback = Math.max(range * Number(rules.minDoubleRejectPullbackRatio || 0.08), tol * 0.65);
      const minReject2 = Math.max(range * Number(rules.minSecondRejectRatio || 0.07), tol * 0.55);
      const retestDistance = Math.abs(secondExtreme - firstExtreme);
      const retestOk = retestDistance <= zone * 1.2 || maxBreak <= tol * 0.35;
      if (!firstNear || !secondNear || !secondNotBreak || !retestOk) continue;
      if (rejection1 < minPullback || rejection2 < minReject2) continue;
      const weakenAtLevel = retestDistance <= zone * 0.9;
      ...
      if (!chosen || (chosenCandidate.rejection1 + chosenCandidate.rejection2) > (chosen.rejection1 + chosen.rejection2)) {
...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (const lvl of levels) {
    ...
    if (distClose > zone * 1.55) continue;

    // La esencia: el precio se desplaza hasta el nivel.
    const attackMove = isResistance ? move : -move;
    if (attackMove < range * rules.minApproachRatio) continue;

    // No debe romper claramente el nivel con cierre del lado incorrecto.
    const pierce = isResistance ? high - level : level - low;
    const closeBeyond = isResistance ? pE - level : level - pE;
    if (closeBeyond > tol * rules.maxCloseBeyondTol) continue;

    const rejection = Math.max(0, isResistance ? high - pE : pE - low);
    ...
    const pierceOk = pierce >= -tol * 0.70;
    const didTouch = isResistance ? high >= level - zone : low <= level + zone;
    if (!didTouch || !pierceOk) continue;

    // v6: el doble rechazo suma mucho, pero NO es obligatorio para que salga señal.
    // Esto permite que la señal nazca cuando ya llegó al nivel y se debilitó/rechazó,
    // y que vos hagas las confirmaciones manuales hasta el segundo 57.
    const doubleReject = analyzeGiroNivelDoubleRejectPattern(pts, level, isResistance, zone, tol, range, rules);
    const weakAtLevel =
      nearScore >= 0.30 &&
      rejectRatio >= Number(rules.minRejectRatio || 0.055) &&
      Math.abs(closeBeyond) <= tol * 0.95;

    const lateStartMs = Math.max(0, evalMs - 12000);
    const pLate = Number(getPriceAtMs(pts, lateStartMs));
    const lateMove = Number.isFinite(pLate) ? pE - pLate : 0;
    const lateStillAttacking = isResistance ? lateMove > range * 0.18 : -lateMove > range * 0.18;
    const clearRejection = rejectRatio >= Number(rules.minRejectRatio || 0.055) * 1.35;

    if (!doubleReject && !weakAtLevel && !clearRejection) continue;
    if (!doubleReject && lateStillAttacking && !clearRejection) continue;

    let points = 0;
    if (touchStrength >= 2) points += 1;
    if (touchStrength >= 3) points += 1;
    if (nearScore >= 0.32) points += 1;
    if (rejectRatio >= rules.minRejectRatio) points += 1;
    if (Math.abs(closeBeyond) <= tol * 0.55) points += 1;
    if (attackMove >= range * 0.28) points += 1;
    if (weakAtLevel) points += 1;

    const firstRejection = doubleReject ? doubleReject.rejection1 : rejection;
    const secondRejection = doubleReject ? doubleReject.rejection2 : 0;
    if (doubleReject) {
      if (doubleReject.rejection1 >= Math.max(range * 0.055, tol * 0.55)) points += 1;
      if (doubleReject.rejection2 >= Math.max(range * 0.045, tol * 0.45)) points += 1;
      if (doubleReject.weakenAtLevel) points += 1;
    }

    if (points < rules.minPoints) continue;

    const direction = isResistance ? "PUT" : "CALL";
    const doubleRejectBonus = doubleReject ? 18 : 0;
    const quality =
      points * 16 +
      touchStrength * 7 +
      nearScore * 18 +
      rejectRatio * 26 +
      (firstRejection / Math.max(range, 1e-9)) * 12 +
      (secondRejection / Math.max(range, 1e-9)) * 16 +
      doubleRejectBonus -
      Math.max(0, closeBeyond / tol) * 10;
    ...
    matches.push({
      ...
      meta: {
        ...
        firstRejection,
        secondRejection,
        firstExtreme: doubleReject ? doubleReject.firstExtreme : (isResistance ? high : low),
        secondExtreme: doubleReject ? doubleReject.secondExtreme : 0,
        weakenAtLevel: doubleReject ? !!doubleReject.weakenAtLevel : !!weakAtLevel,
        hasDoubleReject: !!doubleReject,
        status: doubleReject
          ? "llega al nivel, se debilita, rechazo, nuevo intento y segundo rechazo"
          : "llega al nivel, se debilita y muestra primer rechazo; segundo rechazo se confirma manualmente",
        logic: isResistance
          ? (doubleReject
              ? "giro simple: sube hasta resistencia, se debilita y falla dos veces => PUT"
              : "giro simple: sube hasta resistencia, se debilita y empieza rechazo => PUT")
          : (doubleReject
              ? "giro simple: baja hasta soporte, se debilita y falla dos veces => CALL"
              : "giro simple: baja hasta soporte, se debilita y empieza rechazo => CALL"),
      },
    });
  }
...
function evaluateMinute(minute) {
  ...
  // MODO ÚNICO: Giro Doble Rechazo + IA local por ejemplos.
  // La IA local NO suma puntos de COMPRA/VENTA y NO compra sola.
  // Solo ayuda a decidir si la vela se parece a tus ejemplos buenos y no se parece a las malas.
  if (true) {
    const matches = [];
    for (const c of candidates) {
      const ruleMatch = analyzeGiroNivelSimpleCandidate(c, minute, RULES_GIRO_DOBLE_RECHAZO);
      ...
      // Si la regla de nivel queda corta pero la IA local reconoce la esencia por ejemplos,
      // también deja salir la señal para que vos la confirmes manualmente con +COMPRA/+VENTA.
      if (aiMatch && (!ruleMatch || aiMatch.direction !== ruleMatch.direction)) {
        matches.push({
          ...c,
          direction: aiMatch.direction,
          quality: aiMatch.quality + 8,
          matchSource: "IA_LOCAL",
          giroNivelScore: 0,
          giroNivelPoints: 0,
          giroPolaridadMeta: null,
