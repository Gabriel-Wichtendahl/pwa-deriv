...
const GIRO_NIVEL_LOGIC_VERSION = "GIRO_SNR_CUERPOS_ROLE_REVERSAL_20260510";
...
function evaluateMinute(minute) {
  ...
  // MODO ÚNICO: SNR por cuerpos (soporte/resistencia que cambia de rol) + IA local por ejemplos.
  // La señal exige: recorrido amplio -> llegada débil/irregular -> rechazo fuerte desde el SNR -> respuesta débil del grupo dominante.
  // La IA local NO suma puntos de COMPRA/VENTA y NO compra sola; solo refuerza coincidencias.
  if (true) {
    const matches = [];
    for (const c of candidates) {
      const ruleMatch = analyzeGiroSNRBodyCandidate(c, minute, RULES_GIRO_DOBLE_RECHAZO);
