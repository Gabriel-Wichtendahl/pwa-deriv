...
function drawStudyCaptureToCanvas(canvas, item, nextTicks = []) {
  ...
  ctx.fillStyle = "rgba(220,235,255,.74)";
  ctx.fillText(`${item?.symbol || "—"} · ${dir === "CALL" ? "COMPRA / CALL" : "VENTA / PUT"} · Entrada 57s · ${item?.time || ""}`, 52, 72);

  const resColor = result === "ITM" ? "#43e281" : result === "OTM" ? "#ff5f7a" : "#fbbf24";
  studyDrawPill(ctx, W - 255, 32, 205, 38, `Resultado: ${result}`, resColor);
  ...
  if (allTicks.length >= 2) {
    ...
    if (Number.isFinite(level)) {
      const ly = yOf(level);
      ctx.strokeStyle = isCall ? "#4ade80" : "#ff5878";
      ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(cx0, ly); ctx.lineTo(cx1, ly); ctx.stroke();
      ctx.fillStyle = isCall ? "rgba(74,222,128,.17)" : "rgba(255,88,120,.18)";
      studyRoundRect(ctx, cx0 + 10, ly - 18, 175, 32, 12, true, false);
    ...
    ctx.closePath();
    ctx.fillStyle = "rgba(70,130,165,.22)";
    ctx.fill();
    ...
    const entryQuote = Number(getPriceAtMs(signalTicks.length ? signalTicks : item.ticks, Math.min(60000, Math.max(0, entryMs))));
    const ex = xOf(Math.min(60000, Math.max(0, entryMs)));
    const ey = Number.isFinite(entryQuote) ? yOf(entryQuote) : cy0 + (cy1 - cy0) / 2;
    ctx.strokeStyle = "#facc15";
    ctx.lineWidth = 2;
    ctx.setLineDash([4, 5]);
    ctx.beginPath(); ctx.moveTo(ex, cy0); ctx.lineTo(ex, cy1); ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = "#facc15";
    ctx.beginPath(); ctx.moveTo(ex, ey - 64); ctx.lineTo(ex + 58, ey - 48); ctx.lineTo(ex, ey - 32); ctx.closePath(); ctx.fill();
    ctx.strokeStyle = "#facc15"; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(ex, ey - 12); ctx.lineTo(ex, ey - 64); ctx.stroke();
    ctx.fillStyle = "#211b05";
    ctx.font = "700 13px system-ui, -apple-system, Segoe UI, sans-serif";
    ctx.fillText(dir, ex + 12, ey - 47);
    studyDrawPill(ctx, ex - 56, cy0 + 12, 120, 34, "57s", "#facc15", "#211b05");

    const last = allTicks[allTicks.length - 1];
    const lx = xOf(last.ms), ly2 = yOf(last.quote);
    ctx.fillStyle = resColor;
    ctx.beginPath(); ctx.arc(lx, ly2, 7, 0, Math.PI * 2); ctx.fill();
    studyDrawPill(ctx, Math.max(cx0, lx - 115), ly2 - 52, 95, 34, result, resColor, "#07120d");
  }
  ...
  ctx.font = "500 14px system-ui, -apple-system, Segoe UI, sans-serif";
  ctx.fillStyle = "rgba(220,235,255,.82)";
  const note = `${isCall ? "Soporte" : "Resistencia"} respetad${isCall ? "o" : "a"} · formación señal + vela resultado · ${dir} · ${result}`;
