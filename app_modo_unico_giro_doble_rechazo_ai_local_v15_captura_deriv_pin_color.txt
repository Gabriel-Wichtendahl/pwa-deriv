...
function drawStudyCaptureToCanvas(canvas, item, nextTicks = []) {
  ...
  const tradeColor = isItm ? "#16a34a" : isOtm ? "#ef4444" : "#f59e0b";
  const tradeSoft = isItm ? "rgba(22,163,74,.22)" : isOtm ? "rgba(239,68,68,.22)" : "rgba(245,158,11,.22)";
  const tradeGlow = isItm ? "rgba(34,197,94,.35)" : isOtm ? "rgba(248,113,113,.35)" : "rgba(251,191,36,.30)";
  ...
  for (let sec = 0; sec <= 120; sec += 15) {
    ...
    ctx.strokeStyle = sec === 60 ? "rgba(148,163,184,.35)" : "rgba(148,163,184,.12)";
    ...
    ctx.fillText(`${sec}s`, x - 10, cy1 + 25);
  }
  ...
  if (allTicks.length >= 2) {
    ...
    if (Number.isFinite(level)) {
      const ly = yOf(level);
      const levelColor = isCall ? "#22c55e" : "#f43f5e";
      ctx.strokeStyle = levelColor;
      ctx.lineWidth = 2;
      ctx.globalAlpha = .95;
      ctx.beginPath(); ctx.moveTo(cx0, ly); ctx.lineTo(cx1, ly); ctx.stroke();
      ctx.globalAlpha = 1;
      ctx.fillStyle = isCall ? "rgba(34,197,94,.16)" : "rgba(244,63,94,.16)";
      studyRoundRect(ctx, cx0 + 10, ly - 18, 185, 32, 12, true, false);
    ...
    const closeTick = allTicks.find((p) => p.ms >= 120000) || allTicks[allTicks.length - 1];
    const closeMs = Math.max(safeEntryMs + 1, Number(closeTick.ms || 120000));
    const cx = xOf(closeMs);
    const cy = yOf(Number(closeTick.quote));
    const finishTop = Math.max(cy0 + 40, Math.min(cy - 150, cy0 + 95));

    ctx.shadowColor = tradeGlow;
    ctx.shadowBlur = 10;
    ctx.strokeStyle = tradeColor;
    ctx.lineWidth = 5;
    ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(cx, ey); ctx.stroke();
    ctx.shadowBlur = 0;
    ...
    ctx.save();
    ctx.translate(ex - 46, ey - 92);
    ctx.shadowColor = tradeGlow;
    ctx.shadowBlur = 14;
    ctx.fillStyle = tradeColor;
    ctx.beginPath();
    ctx.arc(0, 0, 30, Math.PI * .88, Math.PI * 2.12);
    ctx.lineTo(0, 58);
    ctx.closePath();
    ctx.fill();
    ctx.shadowBlur = 0;
    ...
    ctx.lineWidth = 4;
    ctx.setLineDash([8, 8]);
    ctx.beginPath(); ctx.moveTo(cx, ey); ctx.lineTo(cx, cy); ctx.stroke();
    ctx.setLineDash([]);

    ctx.fillStyle = tradeColor;
    ctx.beginPath(); ctx.arc(cx, cy, 16, 0, Math.PI * 2); ctx.fill();

    const flagX = Math.min(cx, cx1 - 110);
    const flagY = Math.max(cy0 + 18, finishTop);
    ctx.strokeStyle = tradeColor;
    ctx.lineWidth = 5;
    ctx.beginPath(); ctx.moveTo(flagX, flagY); ctx.lineTo(flagX, flagY + 84); ctx.stroke();
    const fw = 88, fh = 48;
    ctx.fillStyle = tradeSoft;
    ctx.strokeStyle = tradeColor;
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.rect(flagX, flagY, fw, fh);
    ctx.fill(); ctx.stroke();
    const cell = 16;
    for (let r = 0; r < 3; r++) {
      for (let c = 0; c < 5; c++) {
        if ((r + c) % 2 === 0) {
          ctx.fillStyle = tradeColor;
          ctx.fillRect(flagX + c * cell, flagY + r * cell, cell, cell);
        }
      }
    }

    studyDrawPill(ctx, Math.max(cx0, Math.min(cx - 118, cx1 - 130)), cy - 60, 104, 34, result, tradeColor, "#07120d");
    studyDrawPill(ctx, Math.max(cx0, ex - 62), Math.max(cy0 + 12, ey - 140), 126, 34, `${dir} 57s`, tradeColor, "#07120d");
  }
  ...
  const note = `${isCall ? "Soporte" : "Resistencia"} respetad${isCall ? "o" : "a"} · entrada ${dir} 57s · línea, pin, cierre y bandera en ${isItm ? "verde ITM" : isOtm ? "rojo OTM" : "amarillo pendiente"}`;
