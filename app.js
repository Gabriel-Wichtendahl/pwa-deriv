// app.js — Base estable + GIRO NIVEL SIMPLE + GIRO POLARIDAD REAL + LIVE chart FIX + NORMAL+DEBILIDAD FLEX + ✅ Práctica pool sin duplicados/repetición + Trades no quedan colgados (timeouts + race) + ✅ Auto-abrir gráfico (configurable) + ✅ señales activas en Trades/Práctica + ✅ Modo LIKE MANTENIDO
...
const MODE_GIRO_NIVEL = "GIRO NIVEL SIMPLE";
...
const GIRO_NIVEL_LOGIC_VERSION = "GIRO_NIVEL_SIMPLE_MODAL_20260501";
...
function normalizeSignalMode(mode) {
  ...
  if (m === MODE_GIRO_NIVEL || m === "GIRO NIVEL SIMPLE" || m === "GIRO EN NIVEL SIMPLE" || m === "GIRO NIVEL" || m === "GIRO EN NIVEL" || m === "NIVEL SIMPLE" || m === "MODO GIRO NIVEL" || m === "MODO GIRO NIVEL SIMPLE") return MODE_GIRO_NIVEL;
  if (m === MODE_GIRO_POLARIDAD || m === "GIRO POLARIDAD" || m === "GIRO POLARIDAD REAL" || m === "POLARIDAD GIRO" || m === "NIVEL POLARIDAD" || m === "MODO GIRO POLARIDAD" || m === "MODO POLARIDAD" || m === "POLARIDAD" || m === "POLARIDAD REAL") return MODE_GIRO_POLARIDAD;
...
function isGiroFamilyMode(mode) {
  ...
  return m === MODE_GIRO || m === MODE_GIRO_FLEX || m === MODE_GIRO_APRENDIZAJE || m === MODE_GIRO_NIVEL || m === MODE_GIRO_POLARIDAD;
}
function getModeVersion(mode) {
  ...
  if (m === MODE_GIRO_NIVEL) return GIRO_NIVEL_LOGIC_VERSION;
  if (m === MODE_GIRO_POLARIDAD) return GIRO_POLARIDAD_LOGIC_VERSION;
  return "";
}
...
function getModeBtnLabel(mode) {
  ...
  if (m === MODE_GIRO_NIVEL) return "📍 GIRO NIVEL SIMPLE";
  if (m === MODE_GIRO_POLARIDAD) return "🧲 GIRO POLARIDAD REAL";
  return "🟦 Modo NORMAL";
}
function nextSignalMode(mode) {
  ...
  if (m === MODE_GIRO_APRENDIZAJE) return MODE_GIRO_NIVEL;
  if (m === MODE_GIRO_NIVEL) return MODE_GIRO_POLARIDAD;
  if (m === MODE_GIRO_POLARIDAD) return MODE_GIRO;
...
function requestModalDraw(force = false) {
  ...
  modalDrawRaf = requestAnimationFrame(() => {
    ...
    if (modalSub) {
      ...
      modalSub.textContent = `${it.time} | ${getTradeScopeText()} | ticks: ${n}${confTag}${tagLive}${dTag ? " | " + dTag : ""}${tBadge}${autoTag}${polarityTag ? " | " + polarityTag : ""}${c100Tag}`;
    }
...
function getGiroNivelSimpleCandidateLevels(symbol, minute, currentRange, rules = RULES_GIRO_NIVEL_SIMPLE) {
...
function analyzeGiroNivelDoubleRejectPattern(pts, level, isResistance, zone, tol, range, rules = RULES_GIRO_NIVEL_SIMPLE) {
...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_NIVEL_SIMPLE) {
...
function evaluateMinute(minute) {
  ...
  if (signalMode === MODE_GIRO_NIVEL) {
    const matches = [];
    for (const c of candidates) {
      const match = analyzeGiroNivelSimpleCandidate(c, minute, RULES_GIRO_NIVEL_SIMPLE);
      if (!match) continue;
      matches.push({ ...c, direction: match.direction, quality: match.quality, giroNivelScore: Math.round(match.quality), giroNivelPoints: match.points, giroPolaridadMeta: match.meta });
    }
    if (!matches.length) return true;
    matches.sort((a, b) => b.quality - a.quality || b.giroNivelPoints - a.giroNivelPoints || b.giroNivelScore - a.giroNivelScore);
    if (matches.length > 1 && matches[0].quality - matches[1].quality < RULES_GIRO_NIVEL_SIMPLE.minQualityGap) return true;
