...
const GIRO_POLARIDAD_LOGIC_VERSION = "GIRO_POLARIDAD_NIVEL_MODAL_20260501";
...
function normalizeSignalMode(mode) {
  ...
  if (m === MODE_GIRO_POLARIDAD || m === "GIRO EN NIVEL" || m === "GIRO NIVEL" || m === "GIRO POLARIDAD" || m === "POLARIDAD GIRO" || m === "NIVEL POLARIDAD" || m === "MODO GIRO POLARIDAD" || m === "MODO POLARIDAD" || m === "POLARIDAD") return MODE_GIRO_POLARIDAD;
...
function isGiroFamilyMode(mode) {
  ...
  return m === MODE_GIRO || m === MODE_GIRO_FLEX || m === MODE_GIRO_APRENDIZAJE || m === MODE_GIRO_POLARIDAD;
}
...
function getModeBtnLabel(mode) {
  ...
  if (m === MODE_GIRO_POLARIDAD) return "🧲 GIRO POLARIDAD";
  return "🟦 Modo NORMAL";
}
function nextSignalMode(mode) {
  ...
  if (m === MODE_GIRO_APRENDIZAJE) return MODE_GIRO_POLARIDAD;
...
/* =========================
   Giro Polaridad — niveles recientes que el precio no debería pasar
========================= */
...
function getGiroPolarityCandles(symbol, currentMinute = null, lookback = 90) {
...
function getGiroPolarityTolerance(symbol, currentRange = 0) {
  const candles = getGiroPolarityCandles(symbol, null, 30);
  ...
  return Math.max(base * 0.18, avgRange * 0.08, 1e-9);
}
function clusterGiroPolarityLevels(rawLevels, tolerance) {
  const sorted = (rawLevels || []).filter((x) => x && Number.isFinite(Number(x.price))).slice().sort((a, b) => Number(a.price) - Number(b.price));
  const clusters = [];
  for (const lvl of sorted) {
    const price = Number(lvl.price);
    const last = clusters[clusters.length - 1];
    if (!last || Math.abs(price - last.price) > tolerance) {
      clusters.push({ price, type: lvl.type, touches: 1, highs: lvl.type === "resistance" ? 1 : 0, lows: lvl.type === "support" ? 1 : 0, minutes: [lvl.minute] });
    } else {
      const total = last.touches + 1;
      last.price = (last.price * last.touches + price) / total;
      last.touches = total;
      if (lvl.type === "resistance") last.highs += 1;
      if (lvl.type === "support") last.lows += 1;
      last.minutes.push(lvl.minute);
      last.type = last.highs >= last.lows ? "resistance" : "support";
    }
  }
  return clusters;
}
function getGiroPolarityCandidateLevels(symbol, minute, currentRange) {
  ...
  return clusterGiroPolarityLevels(raw, tol * 1.2).map((lvl) => ({ ...lvl, tolerance: tol }));
}
function formatGiroPolarityLevel(item) {
  ...
  const side = pol.levelType === "support" ? "SOPORTE" : "RESISTENCIA";
  ...
  return `🧲 Nivel ${side}: ${Number(pol.level).toFixed(6)} → ${dir}${pts}${touches}`;
}
...
function drawDerivLikeChart(canvas, ticks) {
  ...
  if (modalCurrentItem?.giroPolaridad && Number.isFinite(Number(modalCurrentItem.giroPolaridad.level))) {
    ...
    ctx.font = "12px system-ui, sans-serif";
    const txt = `${pol.levelType === "support" ? "SOPORTE" : "RESIST."} ${level.toFixed(6)}`;
...
function analyzeGiroPolaridadCandidate(candidate, minute, rules = RULES_GIRO_POLARIDAD) {
  ...
  const move = pE - p0;
  const levels = getGiroPolarityCandidateLevels(candidate.symbol, minute, range);
  ...
  for (const lvl of levels) {
    ...
    matches.push({
      ...
      quality,
      points,
      meta: { level, levelType: isResistance ? "resistance" : "support", direction, tolerance: tol, zone, touches: touchStrength, points, distClose, rejectRatio, closeBeyond, high, low, p0, pE, logic: isResistance ? "resistencia/polaridad respetada => giro PUT" : "soporte/polaridad respetada => giro CALL" },
    });
  }
  if (!matches.length) return null;
