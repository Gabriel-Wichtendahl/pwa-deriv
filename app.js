...
const RULES_GIRO_DOBLE_RECHAZO = {
  // v7: secuencia obligatoria según la esencia marcada por Gabriel:
  // 1) grupo dominante se desplaza hasta nivel fuerte,
  // 2) llega irregular / estirado / perdiendo fuerza,
  // 3) grupo contrario entra desde el nivel con rechazo/avance,
  // 4) dominante responde débil,
  // 5) contrario aprovecha ANTES del tiempo de señal.
  minPoints: 6,
  minQualityGap: 4,
  zoneMult: 2.25,
  zoneRangeMult: 0.32,
  minApproachRatio: 0.14,
  minRejectRatio: 0.075,
  minFirstRejectRatio: 0.085,
  minSecondRejectRatio: 0.055,
  minOppositeAdvanceRatio: 0.060,
  maxCloseBeyondTol: 0.70,
  maxDominantResponseRatio: 0.82,
  maxSecondAttemptBreakTol: 0.95,
  minClusterGap: 1,
  minClusterSize: 1,
  approachIrregularityMin: 1.18,
  weakAngleRatio: 0.92,
};
...
function analyzeGiroMandatorySequence(pts, level, isResistance, zone, tol, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  const minApproach = Math.max(range * Number(rules.minApproachRatio || 0.14), tol * 0.85);
  const minFirstReject = Math.max(range * Number(rules.minFirstRejectRatio || rules.minRejectRatio || 0.075), tol * 0.70);
  const minSecondReject = Math.max(range * Number(rules.minSecondRejectRatio || 0.055), tol * 0.50);
  const minOppositeAdvance = Math.max(range * Number(rules.minOppositeAdvanceRatio || 0.06), tol * 0.45);
  ...
  for (let a = 0; a < clusters.length - 1; a++) {
    for (let b = a + 1; b < clusters.length; b++) {
      ...
      if (Math.abs(firstTopZ - levelZ) > zone * 1.25) continue;
      if (Math.abs(secondTopZ - levelZ) > zone * 1.35) continue;

      const preLowIdx = findMinZIndex(arr, 0, firstTopIdx);
      if (preLowIdx < 0 || preLowIdx >= firstTopIdx) continue;
      const approachMove = firstTopZ - arr[preLowIdx].z;
      if (approachMove < minApproach) continue;

      // La llegada debe verse irregular/estirada o perdiendo fuerza.
      const approachPath = sumAbsDeltaZ(arr, preLowIdx, firstTopIdx);
      const approachIrregularity = approachPath / Math.max(approachMove, 1e-9);
      const midApproachIdx = Math.max(preLowIdx + 1, Math.floor((preLowIdx + firstTopIdx) / 2));
      const earlyMove = arr[midApproachIdx].z - arr[preLowIdx].z;
      const lateMove = firstTopZ - arr[midApproachIdx].z;
      const earlyDt = Math.max(1, arr[midApproachIdx].ms - arr[preLowIdx].ms);
      ...
      const approachWeakening = earlySlope > 0 && lateSlope <= earlySlope * Number(rules.weakAngleRatio || 0.92);
      const approachIsValid = approachIrregularity >= Number(rules.approachIrregularityMin || 1.18) || approachWeakening;
      if (!approachIsValid) continue;

      // Grupo contrario entra: rechazo fuerte después del primer ataque al nivel.
      ...
      // El grupo dominante intenta responder, pero lo hace débil.
      const responseAmp = secondTopZ - arr[rejectLowIdx].z;
      if (responseAmp <= 0) continue;
      ...
      const weakResponse = responseRatio <= Number(rules.maxDominantResponseRatio || 0.82) || responseSlope <= rejectionSlope * 0.88;
      if (!weakResponse) continue;

      // Segundo intento no debe romper y sostener arriba/abajo del nivel.
      const secondBreak = secondTopZ - levelZ;
      if (secondBreak > tol * Number(rules.maxSecondAttemptBreakTol || 0.95)) continue;

      // Grupo contrario aprovecha: después de la respuesta débil debe avanzar en contra.
      const afterLowIdx = findMinZIndex(arr, secondTopIdx, arr.length - 1);
      if (afterLowIdx < 0 || afterLowIdx <= secondTopIdx) continue;
      const rejection2 = secondTopZ - arr[afterLowIdx].z;
      const closeAdvantage = secondTopZ - arr[arr.length - 1].z;
      if (Math.max(rejection2, closeAdvantage) < minSecondReject) continue;
      if (closeAdvantage < minOppositeAdvance && rejection2 < minOppositeAdvance * 1.25) continue;

      const closeWrongSide = arr[arr.length - 1].z - levelZ;
      if (closeWrongSide > tol * Number(rules.maxCloseBeyondTol || 0.70)) continue;

      const score =
        approachMove / Math.max(range, 1e-9) * 14 +
        Math.min(2.4, approachIrregularity) * 8 +
        (approachWeakening ? 10 : 0) +
        rejection1 / Math.max(range, 1e-9) * 24 +
        (1 - Math.min(1.25, responseRatio) / 1.25) * 22 +
        rejection2 / Math.max(range, 1e-9) * 24 +
      ...
      const result = {
        ...
        finalExtreme: sign * arr[afterLowIdx].z,
        approachMove,
...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (const lvl of levels) {
    ...
    if (seq.approachMove >= range * Number(rules.minApproachRatio || 0.14)) points += 1;
    if (seq.approachIrregularity >= Number(rules.approachIrregularityMin || 1.18) || seq.approachWeakening) points += 1;
    if (seq.rejection1 >= range * Number(rules.minFirstRejectRatio || 0.085)) points += 1;
    if (seq.weakResponse) points += 1;
    if (seq.rejection2 >= range * Number(rules.minSecondRejectRatio || 0.055)) points += 1;
    if (seq.closeAdvantage >= range * Number(rules.minOppositeAdvanceRatio || 0.060)) points += 1;
    if (Math.abs(closeBeyond) <= tol * Number(rules.maxCloseBeyondTol || 0.70)) points += 1;
    if (points < Number(rules.minPoints || 6)) continue;
    ...
    matches.push({
      ...
      meta: {
        ...
        status: "secuencia completa: aproxima al nivel + debilidad + rechazo + respuesta débil + aprovechamiento contrario",
        logic: isResistance
          ? "compradores llegan a resistencia perdiendo fuerza; vendedores rechazan; compradores responden débil; vendedores aprovechan => PUT"
          : "vendedores llegan a soporte perdiendo fuerza; compradores rechazan; vendedores responden débil; compradores aprovechan => CALL",
      },
    });
  }
...
function evaluateMinute(minute) {
  ...
  // v7: la señal exige la secuencia obligatoria antes del tiempo de señal:
  // aproximación irregular al nivel + rechazo fuerte + respuesta débil + aprovechamiento contrario.
  ...
  if (true) {
    ...
    for (const c of candidates) {
      ...
      // Solo refuerza cuando la secuencia obligatoria de nivel también aparece.
