...
const RULES_GIRO_DOBLE_RECHAZO = {
  ...
  // 6) si evalúa en 35s, con llegar hasta esa respuesta débil alcanza para señal.
...
function analyzeGiroMandatorySequence(pts, level, isResistance, zone, tol, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (let a = 0; a < clusters.length - 1; a++) {
    for (let b = a + 1; b < clusters.length; b++) {
      ...
      // En evaluación de 35s no exigimos el punto 7 completo.
      // La señal puede salir si ya llegó hasta el punto 6:
      // recorrido + llegada débil + primer rechazo + respuesta débil del dominante.
      let afterLowIdx = -1;
      let rejection2 = 0;
      ...
      const result = {
        ...
        closeWrongSide,
        partial35,
        stage: partial35 ? "35s_hasta_respuesta_debil" : "secuencia_completa_doble_rechazo",
        score,
      };
    ...
    matches.push({
      ...
      meta: {
        ...
        status: seq.partial35
          ? "35s: recorrido hasta nivel + pérdida de fuerza + primer rechazo + respuesta débil"
          : "recorrido moderado hasta nivel + pérdida de fuerza + rechazo + respuesta débil + segundo rechazo",
        logic: seq.partial35
          ? (isResistance
              ? "35s: compradores llegan a resistencia, pierden fuerza; vendedores rechazan; compradores reintentan débil => PUT"
              : "35s: vendedores llegan a soporte, pierden fuerza; compradores rechazan; vendedores reintentan débil => CALL")
          : (isResistance
