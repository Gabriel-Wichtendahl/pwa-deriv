...
function analyzeGiroSinNivelSide(pts, isResistance, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  const minAttackFromSwing = range * (allowPartialAt35 ? 0.38 : 0.40);
  const minAttackFromOpen = range * (allowPartialAt35 ? 0.22 : 0.24);
  const minFirstReject = range * (allowPartialAt35 ? 0.145 : 0.165);
  const minRetest = range * (allowPartialAt35 ? 0.055 : 0.060);
  const maxResponseRatio = allowPartialAt35 ? 0.58 : 0.52;
  const maxBreak = range * (allowPartialAt35 ? 0.030 : 0.028);
  ...
  const majorLegs = getGiroMajorMovementLegs(arr, range, allowPartialAt35 ? 0.075 : 0.085);
  if (majorLegs.length < 3 || majorLegs.length > 4) return null;
  if (majorLegs[0].sign !== 1 || majorLegs[1].sign !== -1 || majorLegs[2].sign !== 1) return null;
  if (!allowPartialAt35 && majorLegs.length < 4) return null;
  if (!allowPartialAt35 && majorLegs[3]?.sign !== -1) return null;

  let best = null;

  for (let firstTopIdx = 2; firstTopIdx <= arr.length - 5; firstTopIdx++) {
    const firstTopZ = arr[firstTopIdx].z;
    if (!isNearMajorLegEnd(firstTopIdx, majorLegs[0], 3)) continue;
    if (arr[firstTopIdx].ms < minFirstAttackMs || arr[firstTopIdx].ms > maxFirstAttackMs) continue;
    ...
    for (let rejectLowIdx = firstTopIdx + 1; rejectLowIdx <= arr.length - 4; rejectLowIdx++) {
      ...
      if (!isNearMajorLegEnd(rejectLowIdx, majorLegs[1], 3)) continue;
      ...
      const rejectionHasForce = rejectionSlope >= attackSlope * 0.78 || rejection1 >= range * 0.20;
      if (!rejectionHasForce) continue;
      ...
      if (!isNearMajorLegEnd(secondTopIdx, majorLegs[2], 4)) continue;
      const secondTopZ = arr[secondTopIdx].z;
      ...
      const weakBySpeed = responseSlope <= Math.max(attackSlope * 0.55, rejectionSlope * 0.62);
      const lowerHigh = secondTopZ <= firstTopZ - range * (allowPartialAt35 ? 0.040 : 0.050);
      ...
      if (!allowPartialAt35) {
        ...
        if (majorLegs[3] && !isNearMajorLegEnd(afterLowIdx, majorLegs[3], 4)) continue;
        rejection2 = secondTopZ - arr[afterLowIdx].z;
        ...
        if (closeDominantPush > range * 0.018) continue;
        if (closeAdvantage < range * 0.018 && rejection2 < range * 0.026 && !secondRejectByAngle) continue;
      }
      ...
      const requiredPoints = allowPartialAt35 ? 7 : 9;
