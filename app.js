...
const HIGHLOW_BARRIER_CACHE_KEY = "highLowBarrierCache_v2";
...
function applyExecutionModeUI() {
  ...
  btn.title = shouldUseAutoHighLowExecution()
    ? "Usa barrera relativa cacheada por par: COMPRA = HIGHER y VENTA = LOWER. Si no hay barrera válida, no satura proposals y usa fallback."
...
function formatExecutionPlanMini(plan) {
  if (!plan) return "…";
  const mirror = plan.mirroredBarrier ? " ↔" : plan.symbolDefaultBarrier ? " auto" : " · cache";
