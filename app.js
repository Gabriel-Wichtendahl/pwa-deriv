...
async function findBestHighLowPlan(item, side, opts = {}) {
  ...
  // Esta búsqueda no apunta a 120%: prueba barreras relativas válidas para el símbolo y toma la primera aceptada.
  ...
  try {
    ...
    if (shouldUseAutoHighLowExecution() && itemCtx?.id) {
      ...
      if (!plan?.proposalId || !Number.isFinite(plan.askPrice)) {
        throw new Error(`No encontré propuesta válida para ${side === "CALL" ? "HIGHER" : "LOWER"}. La API usa CALL/PUT + barrier; revisá que el símbolo acepte esa barrera`);
      }
