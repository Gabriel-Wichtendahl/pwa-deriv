...
function getGiroSNRBodyCandidateLevels(symbol, minute, currentRange, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  const clusters = clusterGiroSNRBodyLevels(raw, tol * 0.92);
  const out = [];
  for (const cluster of clusters) {
    ...
    if (touches < 3 && bodyBand > tol * 0.78) continue;
    if (bodyBand > Math.max(tol * 2.2, Math.abs(Number(currentRange || 0)) * 0.18)) continue;
    const breakInfo = getGiroPolarityBreakInfo(candles, cluster, tol * 0.72, { breakCloseTolMult: 0.36 });
    if (!breakInfo) continue;
    const zonePad = Math.min(tol * 0.10, bodyBand * 0.18 + tol * 0.04);
    out.push({
      ...cluster,
      ...breakInfo,
      levelMode: "snr_body",
