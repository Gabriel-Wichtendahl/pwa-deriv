...
const RULES_GIRO_NIVEL_SIMPLE = {
  ...
  zoneMult: 1.75,
  zoneRangeMult: 0.22,
  minApproachRatio: 0.20,
  minRejectRatio: 0.10,
  maxCloseBeyondTol: 0.42,
  minDoubleRejectPullbackRatio: 0.08,
  minSecondRejectRatio: 0.07,
  maxSecondBreakTol: 0.60,
  minClusterGap: 2,
  minClusterSize: 1,
};
...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_NIVEL_SIMPLE) {
  ...
  const minRangeForSymbol = Math.max(evalAbs * 0.000015, 1e-9);
  ...
  for (const lvl of levels) {
    ...
    if (distClose > zone * 1.3) continue;
    ...
    const pierceOk = pierce >= -tol * 0.55;
    ...
    if (!doubleReject) continue;

    let points = 0;
    if (touchStrength >= 2) points += 1;
    if (touchStrength >= 3) points += 1;
    if (nearScore >= 0.45) points += 1;
    if (rejectRatio >= rules.minRejectRatio) points += 1;
    if (Math.abs(closeBeyond) <= tol * 0.35) points += 1;
    if (attackMove >= range * 0.40) points += 1;
    if (doubleReject.rejection1 >= Math.max(range * 0.08, tol * 0.65)) points += 1;
    if (doubleReject.rejection2 >= Math.max(range * 0.07, tol * 0.55)) points += 1;
    ...
    const quality =
      ...
      nearScore * 18 +
      rejectRatio * 22 +
      (doubleReject.rejection1 / Math.max(range, 1e-9)) * 16 +
      (doubleReject.rejection2 / Math.max(range, 1e-9)) * 18 -
      Math.max(0, closeBeyond / tol) * 12;
    matches.push({
      direction,
      quality,
      points,
      meta: {
        ...
        p0,
        pE,
        firstRejection: doubleReject.rejection1,
        secondRejection: doubleReject.rejection2,
        firstExtreme: doubleReject.firstExtreme,
        secondExtreme: doubleReject.secondExtreme,
        weakenAtLevel: !!doubleReject.weakenAtLevel,
        status: "llega al nivel, se debilita, rechazo, nuevo intento y segundo rechazo",
        logic: isResistance
          ? "giro simple: sube hasta resistencia, se debilita, es rechazada y en el segundo intento vuelve a ser rechazada => PUT"
          : "giro simple: baja hasta soporte, se debilita, es rechazado y en el segundo intento vuelve a ser rechazado => CALL",
      },
    });
  }
