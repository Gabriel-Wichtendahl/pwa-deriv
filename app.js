...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_NIVEL_SIMPLE) {
  ...
  for (const lvl of levels) {
    ...
    const quality = points * 18 + touchStrength * 8 + nearScore * 22 + rejectRatio * 25 - Math.max(0, closeBeyond / tol) * 12;
    matches.push({
      direction,
      quality,
      points,
      meta: {
        ...
        p0,
        pE,
        logic: isResistance ? "nivel simple: resistencia respetada => giro PUT" : "nivel simple: soporte respetado => giro CALL",
      },
    });
  }
