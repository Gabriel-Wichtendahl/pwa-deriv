...
const GIRO_NIVEL_LOGIC_VERSION = "GIRO_PATRON_VISUAL_ESTRICTO_20260509";
...
function getPatronVisualLegQuality(arr, legs, i, range, allowPartialAt35) {
  ...
  if (l1EndMs > (allowPartialAt35 ? 24500 : 30000)) return null;
  if (l2EndMs > (allowPartialAt35 ? 31000 : 38000)) return null;
  if (allowPartialAt35 && l3EndMs > evalMs + 500) return null;

  const attackWide = attack >= range * (allowPartialAt35 ? 0.23 : 0.26);
  if (!attackWide) return null;

  // Movimiento 1: amplio, pero con señales de pérdida de fuerza o irregularidad.
  const attackWeakOrIrregular = weakInfo.lateWeakening || weakInfo.irregularity >= 1.14 || weakInfo.efficiency <= 0.82;
  if (!attackWeakOrIrregular) return null;

  // Movimiento 2: el contrario responde mejor. Puede ser fuerte por tamaño, por ángulo o por recuperación clara.
  const rejectionStrongBySize = rejection >= range * (allowPartialAt35 ? 0.135 : 0.155);
  const rejectionBetterAngle = rejectionSlope >= attackSlope * (allowPartialAt35 ? 0.92 : 1.00);
  const rejectionGoodVsAttack = rejection >= attack * (allowPartialAt35 ? 0.42 : 0.48);
  const rejectionStrong = rejectionStrongBySize && (rejectionBetterAngle || rejectionGoodVsAttack);
  ...
  const weakBySize = responseRatio <= (allowPartialAt35 ? 0.72 : 0.64) || responseVsAttack <= 0.42;
  const weakBySpeed = responseSlope <= rejectionSlope * (allowPartialAt35 ? 0.78 : 0.70);
  const doesNotRecoverExtreme = Number(arr[l3.endIdx]?.z || 0) <= Number(arr[l1.endIdx]?.z || 0) + range * (allowPartialAt35 ? 0.045 : 0.030);
  const responseExists = response >= range * 0.035;
  const weakResponse = responseExists && weakBySpeed && (weakBySize || doesNotRecoverExtreme || responseRatio <= 0.50);
  if (!weakResponse) return null;

  // Movimiento 4: para 40s+ se pide aprovechamiento/segundo rechazo.
  let secondRejectOk = true;
  if (!allowPartialAt35) {
    secondRejectOk = !!l4 && l4.sign === -1 && secondReject >= range * 0.055 && (secondRejectSlope >= responseSlope * 0.82 || secondReject >= response * 0.46);
    if (!secondRejectOk) return null;
  }

  // Patrón visual: pocas piernas, orden simple, sin zigzag exagerado.
  const importantCount = legs.filter((l) => Number(l.size || 0) >= range * 0.065).length;
  const maxImportant = allowPartialAt35 ? 4 : 5;
  ...
  const minVisualScore = allowPartialAt35 ? 82 : 95;
  ...
  return {
    ...
    movementFilter: "patron_visual_estricto",
    partial35: allowPartialAt35,
...
function pickPatronVisualEstrictoSequence(arr, range, allowPartialAt35) {
  const minFrac = allowPartialAt35 ? 0.065 : 0.070;
  ...
  return {
    ...
    meta: {
      ...
      status: seq.partial35
        ? "Patrón visual 35s: ataque débil/irregular + rechazo fuerte + respuesta débil"
        : "Patrón visual 40s+: ataque + rechazo + respuesta débil + aprovechamiento",
      logic: seq.isResistance
        ? "patrón visual: comprador pierde fuerza, vendedor responde mejor y comprador vuelve débil => PUT"
        : "patrón visual: vendedor pierde fuerza, comprador responde mejor y vendedor vuelve débil => CALL",
    },
  };
}
...
function evaluateMinute(minute) {
  ...
  // MODO ÚNICO: Patrón visual estricto + IA local por ejemplos.
  // La señal exige una forma parecida al molde: ataque débil/irregular + rechazo fuerte + respuesta débil.
  // La IA local NO suma puntos de COMPRA/VENTA y NO compra sola; solo refuerza coincidencias.
  if (true) {
    const matches = [];
    for (const c of candidates) {
      const ruleMatch = analyzeGiroPatronVisualCandidate(c, minute, RULES_GIRO_DOBLE_RECHAZO);
