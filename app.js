...
// ✅ NUEVO AUTO BACKTEST V10: niveles SNR claros por BLOQUES de 4+ cierres compactos, ideal 5, con un nivel arriba y otro abajo del precio actual
...
const BASE_CONFIG_RESTAURADA_VERSION = "GIRO_AUTO_BACKTEST_DEMO_V10_CLEAR_CLOSE_BLOCKS_20260512";
const AUTO_BACKTEST_VERSION = "GIRO_AUTO_BACKTEST_DEMO_V10_CLEAR_CLOSE_BLOCKS";
...
const AUTO_BACKTEST_REQUIRE_SNR_ROLE_SEQUENCE = true; // V6: secuencia mínima S-R o R-S, no S-R-S/R-S-R
const AUTO_BACKTEST_SNR_ROLE_LOOKBACK = 160;
const AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES = 4;
const AUTO_BACKTEST_SNR_IDEAL_CLOSE_TOUCHES = 5;
const AUTO_BACKTEST_SNR_MAX_ZONE_WIDTH_TOL_MULT = 1.80;
const AUTO_BACKTEST_SNR_PREFER_COMPACT_ZONES = true;
const AUTO_BACKTEST_SNR_REQUIRE_BULL_AND_BEAR_CLOSES = true;
const AUTO_BACKTEST_SNR_CLOSE_CLUSTER_TOL_MULT = 1.10;
const AUTO_BACKTEST_SNR_CLOSE_ZONE_PAD_TOL_MULT = 0.12;
...
const STORE_KEY = "derivSignalsHistory_giroAutoBacktest_v10";
...
========================= */
const TRADES_STORE_KEY = "derivTradesJournal_giroAutoBacktest_v10";
const TRADES_JOURNAL_MAX = 500;

/* =========================
   Capturas de estudio
========================= */
const STUDY_CAPTURE_DB_NAME = "derivStudyCaptures_giroAutoBacktest_v10";
...
========================= */
const ACCOUNT_MODE_KEY = "derivTradingAccountMode_giroAutoBacktest_v10";
...
========================= */
const C100_STATE_KEY = "interesCompuesto2_state_giroAutoBacktest_v10";
...
const EXECUTION_MODE_KEY = "executionMode_giroAutoBacktest_v10";
...
const HIGHLOW_BARRIER_CACHE_KEY = "highLowBarrierCache_giroAutoBacktest_v10_fixed_by_symbol";
const HIGHLOW_BARRIER_CACHE_TTL_MS = 10 * 60 * 1000;
const HIGHLOW_PROPOSAL_COOLDOWN_KEY = "highLowProposalCooldownUntil_giroAutoBacktest_v10";
const HIGHLOW_PROPOSAL_LIMIT_COOLDOWN_MS = 90 * 1000;
const HIGHLOW_DISCOVERY_ATTEMPT_KEY = "highLowDiscoveryAttempt_giroAutoBacktest_v10";
...
========================= */
const AUTOOPEN_CHART_KEY = "autoOpenChartOnSignal_giroAutoBacktest_v10";
...
/* =========================
   Disciplina
========================= */
const DISCIPLINE_WINDOW_START_KEY = "discipline_windowStartMs_giroAutoBacktest_v10";
const DISCIPLINE_WINS_KEY = "discipline_wins_giroAutoBacktest_v10";
const DISCIPLINE_LOSSES_KEY = "discipline_losses_giroAutoBacktest_v10";
const DISCIPLINE_LOCK_UNTIL_KEY = "discipline_lockUntilMs_giroAutoBacktest_v10";
const DISCIPLINE_PENDING_CONTRACTS_KEY = "discipline_pendingContracts_giroAutoBacktest_v10";
...
========================= */
const TRADE_LINKS_KEY = "derivTradeLinks_giroAutoBacktest_v10"; // contract_id -> signalId
...
const ANALYSIS_MODE_KEY = "analysisMode_giroAutoBacktest_v10";
...
const GIRO_POLARIDAD_CANDLES_KEY = "giroPolarityCandles_giroAutoBacktest_v10";
const GIRO_POLARIDAD_MAX_CANDLES = 140;
const GIRO_APRENDIZAJE_STORE_KEY = "giroAprendizajeExamples_giroAutoBacktest_v10";
...
const PRACTICE_SAVED_STORE_KEY = "practiceSavedSignals_giroAutoBacktest_v10";
...
function getGiroPolarityRoleText(pol) {
  ...
  if (pol.levelMode === "snr_body" || pol.levelMode === "snr_close_role_v7_wicks") {
...
let minuteData = {};
// AUTO BACKTEST V6: marca qué minutos/símbolos fueron rehidratados con ticks_history.
...
const LOWPOWER_KEY = "lowPowerMode_giroAutoBacktest_v10";
...
const LIVE_ANALYSIS_PAUSED_KEY = "liveAnalysisPaused_giroAutoBacktest_v10";
...
/* =========================
   Práctica
========================= */
const PRACTICE_STATS_KEY = "practiceStats_giroAutoBacktest_v10";
const PRACTICE_FILTER_KEY = "practiceFilterMode_giroAutoBacktest_v10";
const PRACTICE_POOL_STATE_KEY = "practicePoolState_giroAutoBacktest_v10";
const PRACTICE_EXPORT_SAVED_KEY = "practiceExportSelected_giroAutoBacktest_v10";
...
const PRACTICE_DISPLAY_MODE_KEY = "practiceDisplayMode_giroAutoBacktest_v10";
...
function getSignalSNREntryMeta(item) {
  ...
  if (!meta || typeof meta !== "object") return null;
  const lm = String(meta.levelMode || "");
  if (lm !== "snr_body" && lm !== "snr_close_role_v7_wicks") return null;
  const level = Number(meta.level);
  if (!Number.isFinite(level)) return null;
  return meta;
}
...
function hasAutoBacktestSNRRoleSequenceMeta(item) {
  ...
  if (String(meta.levelMode || "") === "snr_close_role_v7_wicks") {
    if (meta.formedByClose !== true) return false;
    if (Number(meta.closeTouches || 0) < AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES) return false;
    if (AUTO_BACKTEST_SNR_REQUIRE_BULL_AND_BEAR_CLOSES && (Number(meta.bullishCloseTouches || 0) < 1 || Number(meta.bearishCloseTouches || 0) < 1)) return false;
  }

  const pattern = String(meta.rolePattern || "").toUpperCase().trim();
  // V6: alcanza con cambio de rol simple.
...
function normalizeSNRLevelMeta(meta) {
  ...
  return {
    ...
    closeTouches: n(meta.closeTouches, 0),
    bullishCloseTouches: n(meta.bullishCloseTouches, 0),
    bearishCloseTouches: n(meta.bearishCloseTouches, 0),
    formedByClose: !!meta.formedByClose,
    wickFiltered: !!meta.wickFiltered,
    closeZoneLow: n(meta.closeZoneLow),
    closeZoneHigh: n(meta.closeZoneHigh),
    ...
    // AUTO BACKTEST V6:
    // Mantener estos campos es CRÍTICO. En V5 se calculaban bien para el diagnóstico,
    // pero normalizeSNRLevelMeta los descartaba. Entonces addSignal() validaba la señal,
    // no veía rolePattern/roleSequence y la descartaba: el panel decía "Señal creada",
    // pero el contador seguía en 0 y no se intentaba el trade.
    roleTurns: n(meta.roleTurns, 0),
    rolePattern: s(meta.rolePattern),
    roleSequence: s(meta.roleSequence),
    roleEvents: Array.isArray(meta.roleEvents) ? meta.roleEvents : [],
    rolePatternEvents: Array.isArray(meta.rolePatternEvents) ? meta.rolePatternEvents : [],
    close60: n(meta.close60),
    open0: n(meta.open0),
    formationSide: s(meta.formationSide),
    high: n(meta.high),
    low: n(meta.low),
    points: n(meta.points, 0),
    movementFilter: s(meta.movementFilter),
    stage: s(meta.stage),
...
function renderHistory() {
  ...
  for (const it of history || []) {
    ...
    // AUTO BACKTEST V6: esta rama no muestra señales heredadas/tempranas.
...
function finalizeMinute(minute) {
  ...
  // --- AUTO BACKTEST V6: señal + trade nacen recién con cierre 60 REAL confirmado ---
  (async () => {
...
/* =========================
   AUTO BACKTEST V6 — SNR con cambio de rol simple + diagnóstico
   Regla de prueba:
...
function clusterAutoBacktestSNRZones(rawLevels, tolerance) {
  ...
  for (const c of clusters) {
    const rawPrices = Array.isArray(c.rawPrices) ? c.rawPrices.slice().sort((a, b) => a - b) : [];
    const width = rawPrices.length ? Number(c.zoneHigh) - Number(c.zoneLow) : 0;
    const center = Number(c.price);
    const deviations = rawPrices.map((v) => Math.abs(Number(v) - center));
    const avgDeviation = deviations.length ? deviations.reduce((a, b) => a + b, 0) / deviations.length : 0;
    c.zoneWidth = width;
    c.avgDeviation = avgDeviation;
    c.compactness = width / Math.max(Number(tolerance || 0), 1e-9);
    c.clarityScore = Number(c.touches || 0) * 12 - c.compactness * 5 - (avgDeviation / Math.max(Number(tolerance || 0), 1e-9)) * 4;
  }
  return clusters.sort((a, b) => Number(a.price) - Number(b.price));
}
function getAutoBacktestSNRRoleForCandle(candle, zoneLow, zoneHigh, tolerance) {
  const open = Number(candle?.open);
  const close = Number(candle?.close);
  if (![open, close].every(Number.isFinite)) return "";

  const tol = Math.max(Number(tolerance || 0), 1e-12);
  const zl = Math.min(Number(zoneLow), Number(zoneHigh));
  const zh = Math.max(Number(zoneLow), Number(zoneHigh));
  const center = (zl + zh) / 2;

  // V9: el rol del SNR se confirma por CIERRE, no por mecha.
  // Volvemos a permitir mechas como en V7, pero una mecha sola ya no crea/toquea el nivel.
  const closeTouchesZone = close >= zl - tol * 0.18 && close <= zh + tol * 0.18;
  if (!closeTouchesZone) return "";

  // Soporte: viene desde arriba o respeta la zona cerrando del lado superior.
  if (open >= zh + tol * 0.05) return "support";
  // Resistencia: viene desde abajo o respeta la zona cerrando del lado inferior.
  if (open <= zl - tol * 0.05) return "resistance";

  // Si abre dentro de la zona, se usa centro y color de vela como desempate.
  if (close >= center && close >= open) return "support";
  if (close <= center && close <= open) return "resistance";
  return close >= center ? "support" : "resistance";
}
function getAutoBacktestCloseStatsForZone(candles, zoneLow, zoneHigh, tolerance) {
  const tol = Math.max(Number(tolerance || 0), 1e-12);
  const zl = Math.min(Number(zoneLow), Number(zoneHigh));
  const zh = Math.max(Number(zoneLow), Number(zoneHigh));
  const touchCandles = [];
  let bullishCloseTouches = 0;
  let bearishCloseTouches = 0;
  let dojiCloseTouches = 0;

  for (const candle of Array.isArray(candles) ? candles : []) {
    const open = Number(candle?.open);
    const close = Number(candle?.close);
    if (![open, close].every(Number.isFinite)) continue;
    if (close < zl - tol * 0.18 || close > zh + tol * 0.18) continue;
    const dir = close > open ? "bullish" : close < open ? "bearish" : "doji";
    if (dir === "bullish") bullishCloseTouches++;
    else if (dir === "bearish") bearishCloseTouches++;
    else dojiCloseTouches++;
    touchCandles.push({ minute: Number(candle.minute || 0), open, close, dir });
  }

  return {
    touchCandles,
    closeTouches: touchCandles.length,
    bullishCloseTouches,
    bearishCloseTouches,
    dojiCloseTouches,
    hasBullAndBearCloses: bullishCloseTouches > 0 && bearishCloseTouches > 0,
  };
}
function findAutoBacktestNearestCloseLevels(levels, close60) {
  const price = Number(close60);
  const arr = (Array.isArray(levels) ? levels : []).filter((x) => x && Number.isFinite(Number(x.level)) && Number.isFinite(Number(x.zoneLow)) && Number.isFinite(Number(x.zoneHigh)));
  if (!Number.isFinite(price)) return { below: null, above: null };
  const below = arr
    .filter((x) => Number(x.zoneHigh) < price)
    .sort((a, b) => (price - Number(a.zoneHigh)) - (price - Number(b.zoneHigh)))[0] || null;
  const above = arr
    .filter((x) => Number(x.zoneLow) > price)
    .sort((a, b) => (Number(a.zoneLow) - price) - (Number(b.zoneLow) - price))[0] || null;
  return { below, above };
}
...
function getAutoBacktestSNRAlternatingLevels(symbol, minute, currentRange, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (const c of candles) {
    ...
    // V9: el nivel se forma por CIERRES, no por high/low ni por tamaño de mecha.
    // Cierre alcista: sirve para marcar zona de resistencia.
    // Cierre bajista: sirve para marcar zona de soporte.
    raw.push({
      price: close,
      minute: Number(c.minute),
      closeDir: close > open ? "bullish" : "bearish",
      closeRoleHint: close > open ? "resistance" : "support",
    });
  }

  const clusters = clusterAutoBacktestSNRZones(raw, tol * AUTO_BACKTEST_SNR_CLOSE_CLUSTER_TOL_MULT);
  const out = [];
  for (const cluster of clusters) {
    if (Number(cluster.touches || 0) < AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES) continue;

    const rawZoneWidth = Number(cluster.zoneHigh) - Number(cluster.zoneLow);
    if (!Number.isFinite(rawZoneWidth) || rawZoneWidth <= 0) continue;
    if (AUTO_BACKTEST_SNR_PREFER_COMPACT_ZONES && rawZoneWidth > tol * AUTO_BACKTEST_SNR_MAX_ZONE_WIDTH_TOL_MULT) continue;

    const zoneLow = Number(cluster.zoneLow) - tol * AUTO_BACKTEST_SNR_CLOSE_ZONE_PAD_TOL_MULT;
    const zoneHigh = Number(cluster.zoneHigh) + tol * AUTO_BACKTEST_SNR_CLOSE_ZONE_PAD_TOL_MULT;
    if (!Number.isFinite(zoneLow) || !Number.isFinite(zoneHigh) || zoneHigh <= zoneLow) continue;

    const closeStats = getAutoBacktestCloseStatsForZone(candles, zoneLow, zoneHigh, tol);
    if (Number(closeStats.closeTouches || 0) < AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES) continue;
    if (AUTO_BACKTEST_SNR_REQUIRE_BULL_AND_BEAR_CLOSES && !closeStats.hasBullAndBearCloses) continue;
    ...
    out.push({
      ...cluster,
      levelMode: "snr_close_role_v10_clear_blocks",
      formedByClose: true,
      wickFiltered: false,
      minCloseTouches: AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES,
      idealCloseTouches: AUTO_BACKTEST_SNR_IDEAL_CLOSE_TOUCHES,
      compactZone: true,
      requireBullAndBearCloses: AUTO_BACKTEST_SNR_REQUIRE_BULL_AND_BEAR_CLOSES,
      level: Number(cluster.price),
      ...
      bodyZoneLow: Number(cluster.zoneLow),
      bodyZoneHigh: Number(cluster.zoneHigh),
      closeZoneLow: Number(cluster.zoneLow),
      closeZoneHigh: Number(cluster.zoneHigh),
      roleEvents,
      ...
      closeTouches: Number(closeStats.closeTouches || 0),
      bullishCloseTouches: Number(closeStats.bullishCloseTouches || 0),
      bearishCloseTouches: Number(closeStats.bearishCloseTouches || 0),
      dojiCloseTouches: Number(closeStats.dojiCloseTouches || 0),
      closeTouchCandles: closeStats.touchCandles,
      roleTurns: roleEvents.length,
      zoneWidthRaw: rawZoneWidth,
      clusterCompactness: Number(cluster.compactness || 0),
      clarityScore: Number(cluster.clarityScore || 0),
    });
  }
  return out.sort((a, b) =>
    Number(b.closeTouches || 0) - Number(a.closeTouches || 0) ||
    Number(a.zoneWidthRaw || Infinity) - Number(b.zoneWidthRaw || Infinity) ||
...
/* =========================
   AUTO BACKTEST V6 — diagnóstico visible
...
function getAutoBacktestStatusStyle(status) {
  ...
  if (s === "signal_created" || s === "signal_created_trade_attempted" || s === "candidate_ok") return { bg: "rgba(34,197,94,.12)", border: "rgba(34,197,94,.34)", color: "#bbf7d0" };
  if (s === "signal_not_saved") return { bg: "rgba(239,68,68,.12)", border: "rgba(239,68,68,.34)", color: "#fecaca" };
  if (s.includes("outside") || s.includes("no_alternating") || s.includes("no_bull_bear") || s.includes("no_min_touch") || s.includes("momentum") || s.includes("doji")) return { bg: "rgba(251,191,36,.10)", border: "rgba(251,191,36,.30)", color: "#fde68a" };
...
function renderAutoBacktestDiagnosticPanel() {
  ...
  if (summary) {
    ...
    summary.innerHTML = `${escapeHtml(state.message || "Esperando cierre 60s…")}<br><span style="color:rgba(148,163,184,.92)">${minuteTxt} · listos ${Number(state.readySymbols || 0)}/${Number(state.totalSymbols || SYMBOLS.length)} · regla: cierre dentro SNR + cambio de rol S-R/R-S · bloques claros de 4+ cierres (ideal 5)</span>`;
  }
  if (!rows) return;
  ...
  rows.innerHTML = records.map((r, idx) => {
    ...
    const zone = r.closest
      ? `zona ${fmtAutoBacktestNumber(r.closest.zoneLow)}–${fmtAutoBacktestNumber(r.closest.zoneHigh)} · seq ${escapeHtml(r.closest.roleSequence || "—")} · cierres ${Number(r.closest.closeTouches || r.closest.touches || 0)} (${Number(r.closest.bullishCloseTouches || 0)}A/${Number(r.closest.bearishCloseTouches || 0)}B) · ${r.closest.compactZone ? "clara" : "ancha"}`
      : `zonas ${Number(r.compactCloseLevels || 0)}/${Number(r.levelsMinTouch || 0)}/${Number(r.clustersTotal || 0)} · S-R/R-S ${Number(r.alternatingValid || 0)} · cierres ${Number(r.closeLevelsMemory || 0)}`;
    const below = r.nearestBelow ? `abajo ${fmtAutoBacktestNumber(r.nearestBelow.zoneLow)}–${fmtAutoBacktestNumber(r.nearestBelow.zoneHigh)}` : "abajo —";
    const above = r.nearestAbove ? `arriba ${fmtAutoBacktestNumber(r.nearestAbove.zoneLow)}–${fmtAutoBacktestNumber(r.nearestAbove.zoneHigh)}` : "arriba —";
    const sandwich = (r.nearestBelow || r.nearestAbove) ? `<div style="color:rgba(186,230,253,.72);margin-top:2px;">${below} · ${above}</div>` : "";
...
function openAutoBacktestSNRChartFromDiagnostic(index) {
  ...
  if (legend) {
    ...
    legend.innerHTML = `Franja amarilla = zona SNR formada por un bloque claro de cierres. Línea celeste = nivel medio. Punto celeste = cierre 60s. <b>${inside ? "El cierre quedó dentro de la zona." : escapeHtml(record.message || "Diagnóstico")}</b><br>Secuencia: <b>${escapeHtml(seq)}</b> · cierres: <b>${Number(zone.closeTouches || zone.touches || 0)}</b> · alcistas/bajistas: <b>${Number(zone.bullishCloseTouches || 0)}/${Number(zone.bearishCloseTouches || 0)}</b> · zona: <b>${zone.compactZone ? "clara" : "ancha"}</b>`;
  }
...
function getAutoBacktestSNRDiagnosticScan(symbol, minute, currentRange, close60) {
  ...
  for (const cluster of clusters) {
    ...
    const closeTypeOk = !AUTO_BACKTEST_SNR_REQUIRE_BULL_AND_BEAR_CLOSES || closeStats.hasBullAndBearCloses;
    scan.push({
      level: Number(cluster.price),
      touches: Number(roleEvents.reduce((acc, ev) => acc + Number(ev.count || 1), 0)),
      closeTouches: Number(closeStats.closeTouches || 0),
      bullishCloseTouches: Number(closeStats.bullishCloseTouches || 0),
      bearishCloseTouches: Number(closeStats.bearishCloseTouches || 0),
      dojiCloseTouches: Number(closeStats.dojiCloseTouches || 0),
      ...
      inside,
      distance,
      compactZone: rawZoneWidth <= tol * AUTO_BACKTEST_SNR_MAX_ZONE_WIDTH_TOL_MULT,
      rawZoneWidth,
    });
  }

  const minTouch = scan.filter((x) => Number(x.closeTouches || x.touches || 0) >= AUTO_BACKTEST_SNR_MIN_CLOSE_TOUCHES);
  const compactLevels = minTouch.filter((x) => !AUTO_BACKTEST_SNR_PREFER_COMPACT_ZONES || x.compactZone);
  const clearCloseLevels = compactLevels.filter((x) => x.closeTypeOk);
  const alternating = clearCloseLevels.filter((x) => x.alternating);
  const insideAny = clearCloseLevels.filter((x) => x.inside);
  const insideAlternating = alternating.filter((x) => x.inside);
  const nearest = findAutoBacktestNearestCloseLevels(alternating.length ? alternating : clearCloseLevels, close60);
  ...
  return {
    ...
    compactCloseLevels: compactLevels.length,
    clearCloseLevels: clearCloseLevels.length,
...
function diagnoseAutoBacktestSNRCloseCandidate(candidate, minute) {
  ...
  try {
    ...
    let message = "Candidato válido: cierre dentro de zona SNR clara formada por bloques de cierres + cambio de rol";
    if (!direction) {
      status = "blocked_doji";
      ...
      message = "No hay zona SNR con mínimo 4 cierres claros";
    } else if (!scan.compactCloseLevels) {
      status = "blocked_no_compact_zone";
      message = "Hay cierres, pero la zona no quedó clara/compacta";
    } else if (!scan.clearCloseLevels) {
      status = "blocked_no_bull_bear_closes";
      message = "Hay SNR, pero falta mezcla clara de cierres alcistas y bajistas en la zona";
    } else if (!scan.alternatingValid) {
      status = "blocked_no_alternating_roles";
      message = "Hay zona clara por cierres, pero falta cambio de rol S-R/R-S";
    } else if (!scan.closeInsideAlternating) {
      status = "blocked_close_outside_snr";
      message = "El cierre 60s quedó fuera de las zonas SNR claras con cambio de rol";
    ...
    return {
      ...
      compactCloseLevels: scan.compactCloseLevels,
      clearCloseLevels: scan.clearCloseLevels,
      closeLevelsMemory: scan.closeLevelsMemory,
      closeInsideAnySNR: scan.closeInsideAnySNR,
      closeInsideAlternating: scan.closeInsideAlternating,
      nearestBelow: scan.nearestBelow,
      nearestAbove: scan.nearestAbove,
      closest,
      momentum,
    };
  } catch (err) {
...
function summarizeAutoBacktestDiagnostics(records = []) {
  ...
  if (arr.some((r) => r.status === "blocked_no_compact_zone")) return "Descartado: la zona no quedó clara/compacta.";
  if (arr.some((r) => r.status === "blocked_no_bull_bear_closes")) return "Descartado: falta combinación clara de cierres alcistas y bajistas en la zona.";
  if (arr.some((r) => r.status === "blocked_no_alternating_roles")) return "Descartado: SNR por cierres sin cambio de rol S-R/R-S.";
  if (arr.some((r) => r.status === "blocked_no_min_touch")) return "Descartado: no hay SNR con mínimo 4 cierres claros.";
...
function analyzeAutoBacktestSNRCloseCandidate(candidate, minute, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (const lvl of levels) {
    ...
    const clearBonus = Math.min(8, Number(lvl.closeTouches || touches || 0)) * 4;
    const compactBonus = Math.max(0, 16 - Math.min(16, Number(lvl.zoneWidthRaw || 0) / Math.max(Number(lvl.tolerance || 0), 1e-9) * 6));
    const quality = 70 + Math.min(6, roleTurns) * 8 + clearBonus + closeInsideScore * 18 + compactBonus;
    ...
    matches.push({
      ...
      meta: {
        ...
        closeZoneLow: Number(lvl.closeZoneLow || lvl.bodyZoneLow),
        closeZoneHigh: Number(lvl.closeZoneHigh || lvl.bodyZoneHigh),
        formedByClose: true,
        wickFiltered: false,
        closeTouches: Number(lvl.closeTouches || touches || 0),
        bullishCloseTouches: Number(lvl.bullishCloseTouches || 0),
        bearishCloseTouches: Number(lvl.bearishCloseTouches || 0),
        touches,
        roleTurns,
        ...
        low,
        points: Math.round(quality / 12),
        stage: "auto_60_snr_close_role_flip_v10_clear_blocks",
        movementFilter: "snr_clear_close_block_role_flip_close_inside",
        compactZone: true,
        status: `AUTO 60: cierre dentro del SNR por bloque claro de cierres + secuencia ${lvl.rolePattern}`,
        logic: "SNR formado por bloques claros y compactos de 4 o más cierres (ideal 5), con cambio de rol mínimo S-R/R-S; las mechas no forman el nivel.",
      },
    });
  }
  ...
  for (const lvl of levels) {
    ...
    matches.push({
      ...
      meta: {
        ...
        formedByClose: true,
        wickFiltered: false,
        closeTouches: Number(lvl.closeTouches || lvl.touches || 0),
        bullishCloseTouches: Number(lvl.bullishCloseTouches || 0),
        bearishCloseTouches: Number(lvl.bearishCloseTouches || 0),
...
function evaluateMinute(minute) {
  ...
  for (const sym of SYMBOLS) {
    // AUTO BACKTEST V6: no se permite crear señal con ticks vivos/incompletos.
  ...
  const savedSignal = addSignal(minute, bestMatch.symbol, bestMatch.direction, bestMatch.ticks, {
  ...
  if (!savedSignal) {
    const records = diagnostics.map((r) => r.symbol === bestMatch.symbol
      ? { ...r, status: "signal_not_saved", message: "Candidato válido, pero addSignal lo rechazó por validación interna" }
      : r
    ).slice(0, AUTO_BACKTEST_DIAG_RECENT_MAX);
    setAutoBacktestDiagnostic({
      minute,
      message: `⚠️ Candidato válido en ${bestMatch.symbol}, pero no se guardó la señal`,
      readySymbols,
      totalSymbols: SYMBOLS.length,
      records,
    });
  ...
  // AUTO BACKTEST V6 FIX:
  // El trade se intenta inmediatamente con la señal recién guardada. Antes dependía del escaneo
  // posterior y, si addSignal la descartaba silenciosamente, el diagnóstico decía "creada" pero
  // no había fila ni compra. Ahora creación + autoentrada quedan conectadas.
  let autoAttempted = false;
  try {
    autoAttempted = !!trySignalAutoEntryAt57("AUTO_60_CREATED_SIGNAL", savedSignal);
  } catch {}

  const autoStatus = savedSignal?.signalAutoEntry?.status || (autoAttempted ? "sending" : "not_attempted");
  const autoMsg = savedSignal?.signalAutoEntry?.message || savedSignal?.signalAutoEntry?.error || "";
  const records = diagnostics.map((r) => r.symbol === bestMatch.symbol
    ? {
        ...r,
        status: autoStatus === "sending" || autoStatus === "sent" ? "signal_created_trade_attempted" : "signal_created",
        message: autoStatus === "sending"
          ? "Señal creada y trade DEMO enviándose"
          : autoStatus === "sent"
            ? "Señal creada y trade DEMO enviado"
            : autoStatus && autoStatus !== "not_attempted"
              ? `Señal creada; autoentrada: ${autoStatus}${autoMsg ? " · " + autoMsg : ""}`
              : "Señal creada; autoentrada pendiente/no intentada",
      }
    : r
  ).slice(0, AUTO_BACKTEST_DIAG_RECENT_MAX);
  setAutoBacktestDiagnostic({
    minute,
    message: `✅ Señal creada en ${bestMatch.symbol}: ${bestMatch.direction} · AUTO ${autoStatus}`,
    readySymbols,
    totalSymbols: SYMBOLS.length,
    records,
  });
...
function addSignal(minute, symbol, direction, ticks, extra = {}) {
  ...
  // AUTO BACKTEST V6: si no pasa el cierre 60 real dentro del SNR,
  // directamente no se guarda ni se muestra la señal.
  if (!validateAutoBacktestSignalCreatedAtClose(item)) return null;

  const existing = history.find((x) => x && x.id === item.id) || null;
  if (existing) return existing;
