...
function applyLiveAnalysisPauseUI() {
  ...
  btn.textContent = paused ? "▶️ PAUSADO" : "⏸️ LIVE";
  btn.classList.toggle("active", paused);
  btn.setAttribute("aria-pressed", paused ? "true" : "false");
  btn.title = paused
    ? "Análisis en vivo pausado: no se generan señales nuevas. Tocar para reanudar."
    : "Análisis en vivo activo. Tocar para pausar nuevas señales.";
  btn.style.borderColor = paused ? "rgba(248,113,113,.72)" : "rgba(34,211,238,.42)";
  btn.style.background = paused
    ? "linear-gradient(180deg, rgba(127,29,29,.38), rgba(127,29,29,.18))"
    : "linear-gradient(180deg, rgba(34,211,238,.14), rgba(255,255,255,.035))";
  btn.style.color = paused ? "#fecaca" : "#ecfeff";
  btn.style.boxShadow = paused ? "0 0 16px rgba(248,113,113,.18)" : "0 0 14px rgba(34,211,238,.10)";
}
...
function ensureLiveAnalysisPauseButton() {
  ...
  if (!btn) {
    ...
    btn.type = "button";
    btn.className = "tab liveAnalysisPauseBtn";
