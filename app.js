// app.js — MODO ÚNICO: GIRO DOBLE RECHAZO/NIVEL + IA LOCAL POR EJEMPLOS + RECORRIDO MODERADO + IC2 + disciplina visible + auto57 demo/real
...
const RULES_GIRO_DOBLE_RECHAZO = {
  ...
  // 6) contrario rechaza otra vez antes del tiempo de señal.
  minPoints: 7,
  minQualityGap: 4,
  zoneMult: 2.10,
  zoneRangeMult: 0.30,
  ...
  maxFirstAttackMsRatio: 0.86,
  minRouteBodyRatio: 0.48,
  maxRouteIrregularity: 2.85,
  minRejectRatio: 0.075,
  minFirstRejectRatio: 0.085,
  minSecondRejectRatio: 0.040,
  minOppositeAdvanceRatio: 0.050,
  maxCloseBeyondTol: 0.55,
  maxDominantResponseRatio: 0.72,
  maxSecondAttemptBreakTol: 0.55,
  minClusterGap: 1,
  minClusterSize: 1,
  approachIrregularityMin: 1.10,
  weakAngleRatio: 0.82,
  minRejectionSlopeVsApproach: 0.95,
  minSecondRejectSlopeVsResponse: 0.92,
};
...
function analyzeGiroMandatorySequence(pts, level, isResistance, zone, tol, range, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (let a = 0; a < clusters.length - 1; a++) {
    for (let b = a + 1; b < clusters.length; b++) {
      ...
      if (Math.abs(firstTopZ - levelZ) > zone * 1.15) continue;
      if (Math.abs(secondTopZ - levelZ) > zone * 1.25) continue;
      ...
      if (firstBreak > tol * Number(rules.maxSecondAttemptBreakTol || 0.55)) continue;
      ...
      const weakResponse = responseRatio <= Number(rules.maxDominantResponseRatio || 0.72) || responseSlope <= rejectionSlope * 0.78;
      if (!weakResponse) continue;

      // Segundo intento tampoco debe romper y sostener el nivel.
      const secondBreak = secondTopZ - levelZ;
      if (secondBreak > tol * Number(rules.maxSecondAttemptBreakTol || 0.55)) continue;

      // El contrario vuelve a rechazar. Puede ser chico/mediano/grande si el ángulo mejora.
      ...
      const rejection2Slope = rejection2 / rejection2Dt;
      const secondRejectBySize = Math.max(rejection2, closeAdvantage) >= minSecondReject;
      const secondRejectByAngle = rejection2Slope >= responseSlope * Number(rules.minSecondRejectSlopeVsResponse || 0.92) && rejection2 >= tol * 0.30;
      ...
      if (closeWrongSide > tol * Number(rules.maxCloseBeyondTol || 0.55)) continue;
      ...
      const score =
        ...
        rejection2 / Math.max(range, 1e-9) * 22 +
        (secondRejectByAngle ? 12 : 0) +
...
function analyzeGiroNivelSimpleCandidate(candidate, minute, rules = RULES_GIRO_DOBLE_RECHAZO) {
  ...
  for (const lvl of levels) {
    ...
    if (seq.rejection2 >= range * Number(rules.minSecondRejectRatio || 0.040) || seq.secondRejectByAngle) points += 1;
    if (seq.closeAdvantage >= range * Number(rules.minOppositeAdvanceRatio || 0.050) || seq.secondRejectByAngle) points += 1;
    if (Math.abs(closeBeyond) <= tol * Number(rules.maxCloseBeyondTol || 0.55)) points += 1;
    if (points < Number(rules.minPoints || 7)) continue;
    ...
    matches.push({
      ...
      meta: {
        ...
        status: "recorrido moderado hasta nivel + pérdida de fuerza + rechazo + respuesta débil + segundo rechazo",
        logic: isResistance
          ? "compradores hacen recorrido normal/marubozu hasta resistencia, pierden fuerza; vendedores rechazan; compradores reintentan débil; vendedores vuelven a rechazar => PUT"
          : "vendedores hacen recorrido normal/marubozu hasta soporte, pierden fuerza; compradores rechazan; vendedores reintentan débil; compradores vuelven a rechazar => CALL",
      },
    });
  }
