...
function drawStudyCaptureToCanvas(canvas, item, nextTicks = []) {
  ...
  const tradeSoft = isItm ? "rgba(34,197,94,.16)" : isOtm ? "rgba(239,68,68,.16)" : "rgba(245,158,11,.18)";
  const tradeGlow = isItm ? "rgba(34,197,94,.35)" : isOtm ? "rgba(239,68,68,.35)" : "rgba(245,158,11,.35)";
  ...
  if (allTicks.length >= 2) {
    ...
    if (Number.isFinite(level)) {
      const ly = yOf(level);
      const levelColor = isCall ? "rgba(34,197,94,.88)" : "rgba(244,63,94,.88)";
      ctx.strokeStyle = levelColor;
      ctx.lineWidth = 2.2;
      ctx.beginPath(); ctx.moveTo(cx0, ly); ctx.lineTo(cx1, ly); ctx.stroke();
      ctx.fillStyle = isCall ? "rgba(34,197,94,.14)" : "rgba(244,63,94,.14)";
    ...
    ctx.beginPath();
    ctx.moveTo(0, 42);
    ctx.bezierCurveTo(-22, 18, -18, -22, 0, -22);
    ctx.bezierCurveTo(18, -22, 22, 18, 0, 42);
    ctx.closePath();
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#0b1220";
    ...
    studyDrawPill(ctx, Math.max(cx0, Math.min(ex - 48, cx1 - 118)), Math.max(cy0 + 8, pinY - 12), 112, 34, entryTag, tradeColor, "#07120d");

    ctx.shadowColor = tradeGlow;
    ctx.shadowBlur = 8;
    ctx.strokeStyle = tradeColor;
    ctx.lineWidth = 5;
    ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(endX, ey); ctx.stroke();
    ctx.shadowBlur = 0;

    const mastX = endX;
    const mastTopY = Math.max(cy0 + 22, ey - 72);
    const flagW = 68, flagH = 36;
    ctx.strokeStyle = tradeColor;
    ctx.lineWidth = 4;
    ...
    ctx.beginPath(); ctx.moveTo(endX, ey); ctx.lineTo(endX, closeY); ctx.stroke();
    ctx.setLineDash([]);

    ctx.shadowColor = tradeGlow;
    ctx.shadowBlur = 8;
    ctx.fillStyle = tradeColor;
    ctx.beginPath(); ctx.arc(endX, closeY, 17, 0, Math.PI * 2); ctx.fill();
    ctx.shadowBlur = 0;

    const pillW = result === "PEND" ? 118 : 104;
    const pillX = Math.max(cx0, Math.min(endX - 106, cx1 - pillW - 12));
    const pillY = closeY - 58;
    studyDrawPill(ctx, pillX, pillY, pillW, 34, result, tradeColor, "#07120d");

    ctx.fillStyle = "rgba(255,255,255,.42)";
    ctx.font = "700 12px system-ui, -apple-system, Segoe UI, sans-serif";
    ctx.fillText("0-60s Señal", cx0 + 8, cy0 + 16);
    ctx.fillText("60-120s Resultado", xOf(60) + 8, cy0 + 16);
  }
