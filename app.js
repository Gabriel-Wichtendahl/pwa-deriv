...
let activeTab = (localStorage.getItem("activeTab") || "ALL").toUpperCase();
if (!["ALL", "NORMAL", "FUERTE"].includes(activeTab)) activeTab = "ALL";
...
function updateCounter(filteredCount = null) {
  ...
  const shown = filteredCount == null ? total : filteredCount;
  if (!counterEl) return;
  counterEl.textContent =
    activeTab === "ALL" ? `Señales: ${total}` : `Señales: ${shown} (de ${total})`;
}
...
(function initTabs() {
  ...
  function paintTabs() {
    tabBtns.forEach(b => {
      const t = String(b.dataset.tab || "ALL").toUpperCase();
  ...
  tabBtns.forEach(b => {
    b.onclick = () => {
      const t = String(b.dataset.tab || "ALL").toUpperCase();
      activeTab = ["ALL", "NORMAL", "FUERTE"].includes(t) ? t : "ALL";
      localStorage.setItem("activeTab", activeTab);
      paintTabs();
      renderHistory(); // re-render filtrado
    };
  });
})();
...
/* =========================
   ✅ Render historial (con filtro Tabs)
========================= */
function getFilteredHistory() {
  if (activeTab === "ALL") return history;
  return history.filter(it => (it.mode || "NORMAL") === activeTab);
}
...
function renderHistory() {
  ...
  // normalizar modo en históricos viejos
  let changed = false;
  for (const it of history) {
    if (!it.mode) { it.mode = "NORMAL"; changed = true; }
  }
  ...
  // newest first
  for (const it of [...filtered].reverse()) {
...
function openChartModal(item) {
  ...
  // doble RAF para evitar “entrecortado”
...
function drawDerivLikeChart(canvas, ticks) {
  ...
  for (let i = 0; i < pts.length; i++) ctx.lineTo(xOf(pts[i].ms), yOf(pts[i].quote));
...
function addSignal(minute, symbol, direction, ticks) {
  ...
  // render con tabs (si el tab actual no coincide, igual actualiza contador/feedback)
  renderHistory();
