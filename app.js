...
function getExecutionModeLabel() {
  return shouldUseAutoHighLowExecution() ? "🎯 High/Low barrera del par" : "↕️ Rise/Fall 1m";
}
function applyExecutionModeUI() {
  ...
  btn.title = shouldUseAutoHighLowExecution()
    ? "Usa la barrera automática del par: COMPRA = HIGHER y VENTA = LOWER con la barrera espejada."
  ...
  try {
    ...
    if (shouldUseAutoHighLowExecution() && itemCtx?.id) {
      ...
      if (!plan?.proposalId || !Number.isFinite(plan.askPrice)) {
        throw new Error(`No encontré propuesta válida para ${side === "CALL" ? "HIGHER" : "LOWER"}. Probé la barrera del par y búsqueda completa; si sigue, ese símbolo/duración no acepta High/Low en este momento.`);
      }
