...
const HIGHLOW_BARRIER_CACHE_KEY = "highLowBarrierCache_v3_payout150";
...
async function ensureExecutionPlanForTrade(item, side) {
  ...
  cache.error = `Sin proposal válida para ${side === "CALL" ? "HIGHER" : "LOWER"} dentro de ${getHighLowPayoutCapText()}`;
  return null;
}
    ...
    if (shouldUseAutoHighLowExecution() && itemCtx?.id) {
      ...
      if (!plan?.proposalId || !Number.isFinite(plan.askPrice)) {
        ...
        tradeExtra = {
          ...
          payout_total_pct: Number((payout / askPrice) * 100),
